...
class ClassHasDataPrinter(object):
    ...
    def print_class_has_data(self, clazz, leafs, children):
        conditions = ['%s.is_set' % (prop.name)
                      for prop in leafs if not prop.is_many]
        ...
        for child in children:
            if child.is_many:
                self._print_class_has_many(
                    child, 'for (std::size_t index=0; index<%s.len(); index++)', 'if(%s[index]->has_data())' % child.name)
        for leaf in leafs:
            if leaf.is_many:
                self._print_class_has_many(
                    leaf, 'for (auto const & leaf : %s.getYLeafs())', 'if(leaf.is_set)')
        if len(conditions) == 0:
    ...
    def print_class_has_operation(self, clazz, leafs, children):
        ...
        conditions.extend(['ydk::is_set(%s.yfilter)' %
                           (prop.name) for prop in leafs])
        conditions.extend([('(%s !=  nullptr && %s->has_operation())' %
                            (prop.name, prop.name)) for prop in children if not prop.is_many])
        self._print_function_header(clazz, 'has_operation')
        for child in children:
            if child.is_many:
                self._print_class_has_many(
                    child, 'for (std::size_t index=0; index<%s.len(); index++)', 'if(%s[index]->has_operation())' % child.name)
        for leaf in leafs:
            if leaf.is_many:
                self._print_class_has_many(
                    leaf, 'for (auto const & leaf : %s.getYLeafs())', 'if(is_set(leaf.yfilter))')

        self.ctx.writeln('return %s;' % '\n\t|| '.join(conditions))
        self._print_function_trailer()

    def _print_function_header(self, clazz, function_name):
        self.ctx.writeln('bool %s::%s() const' %
                         (clazz.qualified_cpp_name(), function_name))
        self.ctx.writeln('{')
        self.ctx.lvl_inc()
