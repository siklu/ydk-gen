...
class TestEntity : public Entity {
 public:
  TestEntity()
      : name{YType::str, "name"},
        ...
        bits_field{YType::bits, "bits-field"},
 ...
 public:
  bool has_data() const { return name.is_set || enabled.is_set; }

  bool has_operation() const { return false; }

  std::string get_segment_path() const { return "test"; }

  std::vector<std::pair<std::string, LeafData>> get_name_leaf_data() const {
    return {name.get_name_leafdata(), enabled.get_name_leafdata(),
            bits_field.get_name_leafdata()};
  }

  std::shared_ptr<Entity> get_child_by_name(const std::string& child_path,
                                            const string& u) {
  ...
  void set_value(const std::string& name1, const std::string& value,
  ...
  class Child : public Entity {
   public:
    Child() : child_val{YType::int32, "child-val"} {
      yang_name = "child";
      yang_parent_name = "test";
    }

    bool has_data() const { return child_val.is_set; }
    ...
    std::string get_segment_path() const { return "child"; }
    ...
    void set_value(const std::string& leaf_name, const std::string& value,
                   const std::string& value1, const std::string& value2) {
    ...
    class MultiChild : public Entity {
     public:
      MultiChild() : child_key{YType::str, "child-key"} {
        yang_name = "multi-child";
        yang_parent_name = "child";
      }

      bool has_data() const { return child_key.is_set; }
...
TEST_CASE("test_multi_read") {
  ...
  auto m = test.child->get_child_by_name("multi-child",
                                         "multi-child[multi-key='abc']");
  REQUIRE(m != nullptr);
}
