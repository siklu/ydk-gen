...
vector<path::Capability> OpenDaylightCapabilitiesParser::parse(
    vector<string>& capabilities) const {
  vector<path::Capability> path_capabilities{};
  for (const auto& c : capabilities) {
    if (c.find("calvados") != string::npos || c.find("tailf") != string::npos ||
        c.find("tail-f") != string::npos) {
      continue;
    }
  ...
  auto result =
      find(path_capabilities.begin(), path_capabilities.end(), ydk_cap);
  ...
  result = find(path_capabilities.begin(), path_capabilities.end(),
                ietf_netconf_cap);
  if (result == path_capabilities.end()) {
    ...
    if (it.key().find("node-id") != string::npos) {
      ...
      for (auto p = it->begin(); p != it->end(); ++p) {
        for (auto p1 = p->begin(); p1 != p->end(); ++p1) {
          auto a = make_unique<
              ODLNode::AvailableCapabilities::AvailableCapability>();
          a->capability = p1->get<string>();
          odl_node.available_capabilities->available_capability.push_back(
              move(a));
        }
      }
    }
...
static void parse_json(
  ...
  if (network_topology == pt.end()) return;

  auto topology = network_topology->find("topology");
  if (topology == network_topology->end()) return;
...
std::map<std::string, std::unique_ptr<ODLNode>>
OpenDaylightCapabilitiesJsonParser::parse(const string& capabilities_buffer) {
  std::map<std::string, std::unique_ptr<ODLNode>> odl_nodes;
