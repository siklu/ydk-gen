...
class rotating_file_sink : public base_sink<Mutex> {
 public:
  rotating_file_sink(const filename_t& base_filename,
                     const filename_t& extension, std::size_t max_size,
    ...
    for (auto i = _max_files; i > 0; --i) {
      ...
      if (details::file_helper::file_exists(target)) {
        if (details::os::remove(target) != 0) {
          throw spdlog_ex(
              "rotating_file_sink: failed removing " + filename_to_str(target),
              errno);
        }
      }
...
class daily_file_sink : public base_sink<Mutex> {
 public:
  ...
  daily_file_sink(const filename_t& base_filename, const filename_t& extension,
 ...
 protected:
  void _sink_it(const details::log_msg& msg) override {
    if (std::chrono::system_clock::now() >= _rotation_tp) {
      _file_helper.open(
          FileNameCalc::calc_filename(_base_filename, _extension));
      _rotation_tp = _next_rotation_tp();
    }
    _file_helper.write(msg);
  }
