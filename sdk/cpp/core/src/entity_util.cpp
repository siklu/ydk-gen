...
bool is_set(const YFilter& yfilter) { return yfilter != YFilter::not_set; }

static const EntityPath get_entity_path(const Entity& entity,
                                        const string& path_buffer) {
...
static bool is_absolute_path(Entity* ancestor) { return ancestor == nullptr; }
...
//
// @param[in] parent The ancestor relative to which the path is calculated or
// nullptr
// @return EntityPath
...
const EntityPath get_entity_path(const Entity& entity, Entity* ancestor) {
  ...
  if (is_absolute_path(ancestor)) {
    ...
    path_buffer << get_relative_entity_path(&entity, ancestor,
                                            path_buffer.str());
  }
