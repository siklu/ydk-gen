...
const char* PAYLOAD_ERROR_MSG =
    "Codec service only supports one entity per payload, please split payload";

static augment_capabilities_function get_augment_capabilities_function(
    Entity& entity);
static std::string get_bundle_yang_models_location(Entity& entity);
static std::string get_bundle_name(Entity& entity);
static path::RootSchemaNode& get_root_schema(CodecServiceProvider& provider,
                                             Entity& entity);
...
std::string CodecService::encode(CodecServiceProvider& provider, Entity& entity,
                                 bool pretty, bool subtree) {
  ...
  std::string result =
      core_codec_service.encode(*dn, provider.m_encoding, pretty);
  YLOG_INFO("Performing encode operation, resulting in {}", result);
  return result;
}

std::map<std::string, std::string> CodecService::encode(
    CodecServiceProvider& provider,
    std::map<std::string, std::unique_ptr<Entity>>& entity_map, bool pretty) {
...
std::shared_ptr<Entity> CodecService::decode(CodecServiceProvider& provider,
  ...
  path::Codec core_codec_service{};
  auto root_data_node =
      core_codec_service.decode(root_schema, payload, provider.m_encoding);
...
std::map<std::string, std::shared_ptr<Entity>> CodecService::decode(
    CodecServiceProvider& provider,
    std::map<std::string, std::string>& payload_map,
    std::map<std::string, std::shared_ptr<Entity>> entity_map) {
  for (auto it : payload_map) {
    std::shared_ptr<Entity> entity =
        decode(provider, it.second, entity_map[it.first]);
...
static std::string get_bundle_yang_models_location(Entity& entity) {
  if (entity.parent == nullptr) return entity.get_bundle_yang_models_location();

  return get_bundle_yang_models_location(*(entity.parent));
}

static std::string get_bundle_name(Entity& entity) {
  if (entity.parent == nullptr) return entity.get_bundle_name();
...
static path::RootSchemaNode& get_root_schema(CodecServiceProvider& provider,
  ...
  provider.initialize(bundle_name, get_bundle_yang_models_location(entity),
                      get_augment_capabilities_function(entity));

  path::RootSchemaNode& root_schema =
      provider.get_root_schema_for_bundle(bundle_name);
  return root_schema;
}
}  // namespace ydk
