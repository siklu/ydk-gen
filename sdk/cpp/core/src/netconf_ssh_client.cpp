...
NetconfSSHClient::NetconfSSHClient(string username, string private_key_path,
                                   string public_key_path, string server_ip,
...
string NetconfSSHClient::execute_payload(const string& payload) {
  perform_session_check("Cannot not execute payload. Not connected to " +
                        hostname);
  ...
  perform_session_check("Cannot not receive RPC reply. Not connected to " +
                        hostname);
...
NetconfSSHClient::~NetconfSSHClient() { nc_session_free(session); }
...
StringVec NetconfSSHClient::get_capabilities() { return capabilities; }
...
char* NetconfSSHClient::clb_set_password(const char* user_name,
  ...
  char* password_buffer =
      (char*)malloc(sizeof(char) * (password_string.size() + 1));
...
char* NetconfSSHClient::clb_set_interactive(const char* name,
...
char* NetconfSSHClient::clb_set_passphrase(const char* user_name,
                                           const char* host_name,
...
void NetconfSSHClient::perform_session_check(const string& message) {
  ...
  if (session_status != NC_SESSION_STATUS_WORKING &&
      session_status != NC_SESSION_STATUS_CLOSING) {
