...
static void create_input_target(path::DataNode& input,
...
static shared_ptr<path::DataNode> handle_crud_edit_reply(
...
static string get_netconf_payload(path::DataNode& input, const string& data_tag,
...
shared_ptr<path::DataNode> handle_rpc_output(const string& reply,
                                             path::RootSchemaNode& root_schema,
                                             const string& rpc_path);
shared_ptr<path::DataNode> handle_action_output(
    const string& reply, path::RootSchemaNode& root_schema,
...
static shared_ptr<path::DataNode> netconf_output_to_datanode(
    const string& data, path::RootSchemaNode& root_schema);
static string extract_rpc_output(const string& reply);
static string extract_rpc_data(const string& reply, const string& start_tag,
                               const string& end_tag,
                               bool is_first_tag = false);

NetconfSession::NetconfSession(path::Repository& repo, const string& address,
                               const string& username, const string& password,
                               int port, const string& protocol, bool on_demand,
                               int timeout) {
  ...
  YLOG_INFO("Connected to {} on port {} using {} with timeout of {}", address,
            port, protocol, timeout);
}

NetconfSession::NetconfSession(const string& address, const string& username,
                               const string& password, int port,
                               const string& protocol, bool on_demand,
                               bool common_cache, int timeout) {
  // Correct default settings
  if (port == 0) port = 830;

  initialize_client(address, username, password, port, protocol, timeout);
  auto caching_option = common_cache ? path::ModelCachingOption::COMMON
                                     : path::ModelCachingOption::PER_DEVICE;
...
NetconfSession::NetconfSession(path::Repository& repo, const string& address,
                               ...
                               const string& public_key_path, int port,
                               bool on_demand, int timeout) {
  // Correct default settings
  if (port == 0) port = 830;

  initialize_client_with_key(address, username, private_key_path,
                             public_key_path, port, timeout);
  initialize_repo(repo, on_demand);
  YLOG_INFO("Connected to {} on port {} using SSH with timeout of {}", address,
            port, timeout);
}
...
NetconfSession::NetconfSession(const string& address, const string& username,
                               const string& private_key_path,
                               const string& public_key_path, int port,
                               bool on_demand, bool common_cache, int timeout) {
  // Correct default settings
  if (port == 0) port = 830;
...
void NetconfSession::initialize_client_with_key(const string& address,
  ...
  //        username, private_key_path, public_key_path, address, port,
  //        timeout);
}
...
void NetconfSession::initialize_client(const string& address,
                                       const string& username,
                                       const string& password, int port,
                                       const string& protocol, int timeout) {
  if (protocol.compare(PROTOCOL_SSH) == 0 || protocol.empty()) {
    //        client = make_shared<NetconfSSHClient>(username, password,
    //        address, port, timeout);
  } else if (protocol.compare(PROTOCOL_TCP) == 0) {
    //        client = make_shared<NetconfTCPClient>(username, password,
    //        address, port);
  } else {
...
NetconfSession::~NetconfSession() { YLOG_INFO("Disconnected from device"); }
...
shared_ptr<path::DataNode> NetconfSession::handle_crud_read(
    path::Rpc& ydk_rpc) const {
  // for now we only support crud rpc's
  bool config = is_config(ydk_rpc);
  auto netconf_rpc =
      create_rpc_instance(*root_schema, get_read_rpc_name(config));
...
shared_ptr<path::DataNode> NetconfSession::handle_crud_edit(
    path::Rpc& ydk_rpc, path::Annotation annotation) const {
  // for now we only support crud rpc's
  bool candidate_supported = is_candidate_supported(server_capabilities);

  auto netconf_rpc =
      create_rpc_instance(*root_schema, "ietf-netconf:edit-config");
  auto& input = create_rpc_input(*netconf_rpc);
  create_input_target(input, candidate_supported);
  create_input_error_option(input);
  string config_payload =
      get_annotated_config_payload(*root_schema, ydk_rpc, annotation);
  ...
  return handle_crud_edit_reply(execute_payload(netconf_payload), *client,
                                candidate_supported);
}

shared_ptr<path::DataNode> NetconfSession::handle_netconf_operation(
    path::Rpc& ydk_rpc) const {
  path::Codec codec_service{};
  auto netconf_payload =
      codec_service.encode(ydk_rpc.get_input_node(), EncodingFormat::XML, true);
  ...
  if (is_netconf_get_rpc(ydk_rpc)) {
    ...
    if (!data.empty()) return netconf_output_to_datanode(data, *root_schema);
  } else if (ydk_rpc.has_output_node()) {
    return handle_rpc_output(reply, *root_schema,
                             ydk_rpc.get_input_node().get_path());
  }
  return nullptr;
}

std::string NetconfSession::execute_netconf_operation(
    path::Rpc& ydk_rpc) const {
  path::Codec codec_service{};
  auto netconf_payload =
      codec_service.encode(ydk_rpc.get_input_node(), EncodingFormat::XML, true);
...
shared_ptr<path::DataNode> NetconfSession::invoke(
    path::DataNode& datanode) const {
  if (!datanode.has_action_node()) {
    YLOG_ERROR("Datanode {} does not contain any action nodes",
               datanode.get_path());
    throw(YServiceProviderError{"Datanode does not contain any action nodes: " +
                                datanode.get_path()});
  }
  path::Codec codec_service{};
  auto netconf_payload =
      codec_service.encode(datanode, EncodingFormat::XML, true);

  netconf_payload =
      "<rpc xmlns=\"urn:ietf:params:xml:ns:netconf:base:1.0\"><action "
  ...
  return handle_action_output(reply, *root_schema,
                              datanode.get_action_node_path());
}

shared_ptr<path::DataNode> NetconfSession::invoke(path::Rpc& rpc) const {
  path::SchemaNode* create_schema =
      get_schema_for_operation(*root_schema, "ydk:create");
  path::SchemaNode* read_schema =
      get_schema_for_operation(*root_schema, "ydk:read");
  path::SchemaNode* update_schema =
      get_schema_for_operation(*root_schema, "ydk:update");
  path::SchemaNode* delete_schema =
      get_schema_for_operation(*root_schema, "ydk:delete");
  ...
  if (rpc_schema == create_schema || rpc_schema == delete_schema ||
    ...
    path::Annotation an{IETF_NETCONF_MODULE_NAME, "operation",
...
static bool is_yang_1_1_supported(vector<string>& caps) {
  for (string& c : caps) {
    if (c.find("urn:ietf:params:netconf:capability:yang-library:1.0") !=
        string::npos) {
      return true;
    }
  }
...
static string get_annotated_config_payload(path::RootSchemaNode& root_schema,
  ...
  auto datanode =
      codec_service.decode(root_schema, entity_value, EncodingFormat::XML);
...
static bool is_netconf_get_rpc(path::Rpc& rpc) {
  return (rpc.get_schema_node().get_path() == "/ietf-netconf:get" or
          rpc.get_schema_node().get_path() == "/ietf-netconf:get-config");
}

std::string get_netconf_output(const string& reply) {
  if (reply.find("<data/>") != string::npos ||
      reply.find("<nc:data/>") != string::npos) {
    YLOG_INFO(
        "Found empty data tag, meaning requested data are not found on Netconf "
        "server");
    return {};
  }
...
static shared_ptr<path::DataNode> netconf_output_to_datanode(
    const string& data, path::RootSchemaNode& root_schema) {
  path::Codec codec_service{};
  auto datanode = shared_ptr<path::DataNode>(
      codec_service.decode(root_schema, data, EncodingFormat::XML));
...
shared_ptr<path::DataNode> handle_rpc_output(const string& reply,
  ...
  shared_ptr<path::DataNode> datanode = Codec().decode_rpc_output(
      root_schema, data, rpc_path, EncodingFormat::XML);
  return datanode;
}
...
shared_ptr<path::DataNode> handle_action_output(
  ...
  if (data.length() == reply.length()) {
    ...
    if (data.length() == reply.length()) {
      YLOG_INFO(
          "Could not locate start and/or end 'data' tag in the RPC reply");
      return nullptr;
    }
  }

  shared_ptr<path::DataNode> datanode = Codec().decode_rpc_output(
      root_schema, data, action_node_path, EncodingFormat::XML);
  return datanode;
}
