...
OpenDaylightServiceProvider::OpenDaylightServiceProvider(
    ...
    const string& password, int port, EncodingFormat encoding,
    Protocol protocol)
    : m_repo(repo),
      address(address),
      username(username),
  ...
  RestconfClient client{address, username, password, port,
                        get_encoding_string(encoding)};

  OpenDaylightCapabilitiesJsonParser odl_parser{};
  string odl_caps_xml =
      client.get_capabilities("/operational/network-topology:network-topology/",
                              get_encoding_string(EncodingFormat::JSON));
...
unique_ptr<ydk::ServiceProvider>
OpenDaylightServiceProvider::create_provider_for_node(const string& node_id) {
  ...
  return make_unique<RestconfServiceProvider>(
      make_unique<RestconfClient>(address, username, password, port,
                                  get_encoding_string(encoding)),
      m_repo.create_root_schema(capabilities), "PUT",
      string("/config/network-topology:network-topology/topology/"
             "topology-netconf/node/") +
          node_id + string("/yang-ext:mount"),
      string("/operational/network-topology:network-topology/topology/"
             "topology-netconf/node/") +
          node_id + string("/yang-ext:mount"),
      encoding);
}

ydk::ServiceProvider& OpenDaylightServiceProvider::get_node_provider(
