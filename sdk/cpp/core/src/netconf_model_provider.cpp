...
namespace path {
static string get_static_model(const std::string& name,
...
string NetconfModelProvider::get_model(const string& name,
                                       const string& version, Format format) {
...
StaticModelProvider::StaticModelProvider(NetconfClient& client)
    : client(client) {}
...
string StaticModelProvider::get_model(const string& name, const string& version,
                                      Format format) {
  (void)format;
...
static string get_static_model(const std::string& name,
                               const std::string& version) {
  if (name == ydk::path::YDK_MODULE_NAME &&
      version == ydk::path::YDK_MODULE_REVISION) {
    return ydk::path::YDK_MODULE;
  }

  if (name == ydk::IETF_NETCONF_MODULE_NAME &&
      version == ydk::IETF_NETCONF_MODULE_REVISION) {
