  ...
  switch (level) {
    case LY_LLERR:
      if (error_message.str().find("Invalid value") != std::string::npos ||
          ...
          error_message.str().find("Unexpected character") !=
              std::string::npos ||
          error_message.str().find("does not satisfy the constraint") !=
              std::string::npos) {
        YLOG_ERROR(
            "Data is invalid according to the yang model. Libyang error: {}",
            error_message.str());
        throw(YModelError{});
      }
      YLOG_ERROR(
...
void set_libyang_logging_callback() { ly_set_log_clb(libyang_log_callback, 1); }

void set_logging_callback(const char* level, logging_callback func) {
  if (!strcmp(level, trace)) {
    ydk_logging_trace_function = func;
  }
  if (!strcmp(level, debug)) {
    ydk_logging_debug_function = func;
  }
  if (!strcmp(level, info)) {
    ydk_logging_info_function = func;
  }
  if (!strcmp(level, warn)) {
    ydk_logging_warn_function = func;
  }
  if (!strcmp(level, error)) {
    ydk_logging_error_function = func;
  }
  if (!strcmp(level, critical)) {
    ydk_logging_critical_function = func;
  }
}

logging_callback get_logging_callback(const char* level) {
  if (!strcmp(level, trace)) {
    return ydk_logging_trace_function;
  }
  if (!strcmp(level, debug)) {
    return ydk_logging_debug_function;
  }
  if (!strcmp(level, info)) {
    return ydk_logging_info_function;
  }
  if (!strcmp(level, warn)) {
    return ydk_logging_warn_function;
  }
  if (!strcmp(level, error)) {
    return ydk_logging_error_function;
  }
  if (!strcmp(level, critical)) {
    return ydk_logging_critical_function;
  }
  return nullptr;
}
