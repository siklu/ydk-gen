...
/// - Codec that can encode a DataNode to XML/JSON and decode from XML/JSON to a
/// dataNode tree
///
...
/// Memory management of trees. DO NOT get rid of the SchemaTree if there are
/// DataTree's referencing it Thread safety Inspecting YANG meta data Traversing
/// the hierarchy (iterations and find)
///
...
/// Full XPath notation is supported for find operations on DataNode(s). This
/// XPath conforms to the YANG specification (RFC 6020 section 6.4). Some useful
/// examples:
///
...
///    __key2__ of value __2__ (this can return more __list__ instances if there
///    are more keys than __key1__ and __key2__)
///
...
///
/// - get __aug-leaf__, which was added to __module-name__ from an augment
/// module __augment-module__
///
...
/// Basically, only a relative or absolute path can be specified to identify a
/// new data node. However, lists must be identified by all their keys and
/// created with all of them, so for those cases predicates are allowed.
/// Predicates must be ordered the way the keys are ordered and all the keys
/// must be specified. Every predicate includes a single key with its value.
/// Optionally, leaves and leaf-lists can have predicates specifying their value
/// in the path itself. All these paths are valid XPath expressions. Example:
/// (Relative to Root Data or RootSchemaNode)
///
/// ietf-yang-library:modules-state/module[name='ietf-yang-library'][revision='']/conformance[.='implement']
///
/// Almost the same XPath is accepted by SchemaNode methods. The difference is
/// that it is not used on data, but schema, which means there are no key values
/// and only one node matches one path. In effect, lists do not have to have any
/// predicates. If they do, they do not need to have all the keys specified and
/// if values are included, they are ignored. Nevertheless, any such expression
/// is still a valid XPath, but can return more nodes if executed on a data
/// tree. Examples (all returning the same node):
///
...
///
///
/// Also note, that in all cases the node's prefix is specified as the name of
/// the appropriate YANG schema. Any node can be prefixed by the module name.
/// However, if the prefix is omitted, the module name is inherited from the
/// previous (parent) node. It means, that the first node in the path is always
/// supposed to have a prefix.
///

///
/// @page howtorpc Rpc
/// An Rpc represents an instance of the YANG Rpc schema node.
/// To invoke and Rpc the user first creates and Rpc using the
/// RootSchema#create_rpc call passing in a path with the name of the rpc For
/// example auto get_config = root_schema->create_rpc("ietf-netconf:get-config")
///
/// The input DataNode can be obtained using Rpc#input. This can be used to
/// populate/create the child nodes of input as per this rpc's schema. The Rpc
/// is a callable that takes  a single argument which is the Session. To invoke
/// the rpc do this auto config = get_config(session); The config variable above
/// is the DataNode representing the output of the rpc
///
///
...
/// A given DataNode Tree can be encoded and decoded into a variety of formats
/// using the Codec.
//
/// DataNode Tree can be validated using the ValidationService
///
...
enum class ModelCachingOption { COMMON, PER_DEVICE };
...
class Codec {
 public:
  ...
  std::string encode(std::vector<ydk::path::DataNode*>& data_nodes,
                     ydk::EncodingFormat format, bool pretty);
  ...
  /// @throws YInvalidArgumentError if the arguments are invalid.
  ///
  std::shared_ptr<DataNode> decode(RootSchemaNode& root_schema,
                                   const std::string& buffer,
                                   EncodingFormat format);
  std::shared_ptr<DataNode> decode_json_output(
      RootSchemaNode& root_schema, const std::vector<std::string>& buffer_list);
  std::shared_ptr<DataNode> decode_rpc_output(RootSchemaNode& root_schema,
                                              const std::string& buffer,
...
struct YDataValidationError : public YCoreError {
  ...
  /// List of pair<DataNode, ValidationError>. The Validation Error is specific
  /// to this node
...
struct YPathError : public YCoreError {
  enum class Error {
    ...
    PATH_AMBIGUOUS  //// thrown in create where the path expression cannot
                    /// uniquely identify a given node
  };
...
/// Annotations are not defined in the YANG model and hence just provide a means
/// of hanging some useful data to DataNodes. For example netconf edit-config
/// rpc operation uses the annotation nc:operation (nc refers to the netconf
/// namespace) on the data nodes to describe the kind of operation one needs to
/// perform on the given DataNode.
///
struct Annotation {
  Annotation(const std::string& ns, const std::string& name,
...
class SchemaNode {
 public:
  ...
  /// @throws YPathError if the path expression in invalid, See error code for
  /// details.
  ...
  virtual const std::vector<std::unique_ptr<SchemaNode>>& get_children()
      const = 0;
...
class RootSchemaNode : public SchemaNode {
 public:
  ...
  /// expression must identify a single node. If the last node created is of
  /// schema type list, leaf-list or anyxml that value is also set in the node.
  /// The returned DataNode is the last node created (the terminal part of the
  /// path). The user is responsible for managing the memory of this returned
  /// tree. Use DataNode#get_root() to get the root element of the this tree and
  /// use that pointer to dispose of the entire tree. Note in the case of List
  /// nodes the keys must be present in the path expression in the form of
  /// predicates.
  ///
  /// @param[in] path The XPath expression identifying the node relative to the
...
class DataNode {
 public:
  ...
  /// Note the DataNode should represent a leaf , leaf-list or anyxml element
  /// for this to work. The value should be the string representation of the
  /// type of according to the schema. Note this method does not validate the
  /// value being set. To validate please see the ValidationService.
  ///
  /// @param[in] value The value to set. This should be the string
  /// representation of the YANG type.
  /// @throws YInvalidArgumentError if the DataNode's value cannot be set (for
  /// example it represents a container)
  ...
  virtual std::vector<std::shared_ptr<DataNode>> find(
...
class ModelProvider {
 public:
  ...
  /// @return string containing the data of the model downloaded. If empty then
  /// the model is probably cannot be provided
  ///
  virtual std::string get_model(const std::string& name,
                                const std::string& version, Format format) = 0;
...
///
/// A instance of the Repository will be used to create a RootSchemaNode given a
/// set of Capabilities. Behind the scenes the repository is responsible for
/// loading and parsing the YANG modules and creating the SchemaNode tree.
/// Sessions are expected to use the Repository#create_root_schema to generate
/// the RootSchemaNode.
///
class Repository {
 public:
  ...
  Repository(
      ModelCachingOption caching_option = ModelCachingOption::PER_DEVICE);

  ///
  /// @brief Constructor for the Repository.
  ///
  /// Constructor
  /// @param[in] search_dir The path in the filesystem where yang files can be
  /// found.
  /// @throws YInvalidArgumentError if the search_dir is not a valid directory
  /// in the filesystem
  Repository(const std::string& search_dir, ModelCachingOption caching_option =
                                                ModelCachingOption::PER_DEVICE);
  ...
  /// @return pointer to the RootSchemaNode or nullptr if one could not be
  /// created.
  ///
  std::shared_ptr<RootSchemaNode> create_root_schema(
      const std::vector<Capability>& capabilities);
  std::shared_ptr<RootSchemaNode> create_root_schema(
      const std::unordered_map<std::string, Capability>& lookup_table,
...
/// Use the Repository class to instantiate a SchemaTree based on the
/// Capabilities.
class Session {
 public:
  virtual ~Session();
  ...
  ///
  /// invokes or executes the action defined inside the given DataNode and
  /// Returns a DataNode pointer if the action has an output modelled in YANG.
  ///
  /// @param[in] pointer to the Rpc node
  /// @return The pointer to the DataNode representing the output.
  ///
...
class NetconfSession : public Session {
 public:
  NetconfSession(Repository& repo, const std::string& address,
                 const std::string& username, const std::string& password,
                 int port = 830, const std::string& protocol = "ssh",
                 bool on_demand = true, int timeout = -1);

  NetconfSession(const std::string& address, const std::string& username,
                 const std::string& password, int port = 830,
                 const std::string& protocol = "ssh", bool on_demand = true,
                 bool common_cache = false, int timeout = -1);
  ...
  std::shared_ptr<path::DataNode> handle_action_rpc_output(
      const std::string& rpc_reply, path::DataNode& action_dn);
  void check_session_state();  // throws YClientError exception when state is
 ...
 private:
  ...
  void initialize_client_with_key(const std::string& address,
                                  const std::string& username,
                                  const std::string& private_key_path,
                                  const std::string& public_key_path, int port,
                                  int timeout);
...
class RestconfSession : public Session {
 public:
  RestconfSession();
  RestconfSession(Repository& repo, const std::string& address,
                  const std::string& username, const std::string& password,
                  int port = 80, EncodingFormat encoding = EncodingFormat::JSON,
  ...
  RestconfSession(std::shared_ptr<RestconfClient> client,
                  ...
                  const std::string& edit_method, EncodingFormat encoding,
 ...
 private:
  ...
  std::shared_ptr<DataNode> handle_crud_edit(Rpc& rpc,
                                             const std::string& yfilter) const;
  std::shared_ptr<DataNode> handle_crud_read(Rpc& rpc) const;
...
/// as its parameter returning a pointer to a DataNode tree if output is
/// available
///
class Rpc {
 public:
