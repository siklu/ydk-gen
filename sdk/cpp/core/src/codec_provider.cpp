...
CodecServiceProvider::CodecServiceProvider(path::Repository& repo,
                                           EncodingFormat encoding)
    : m_encoding{encoding},
      user_provided_repo(true),
      capabilities_augmented(false),
      m_repo(repo) {}

CodecServiceProvider::CodecServiceProvider(EncodingFormat encoding)
    : m_encoding{encoding},
      user_provided_repo(false),
      capabilities_augmented(false) {}

CodecServiceProvider::~CodecServiceProvider() {}

void CodecServiceProvider::initialize(
    const std::string& bundle_name, const std::string& models_path,
    augment_capabilities_function augment_caps_function) {
...
path::RootSchemaNode& CodecServiceProvider::get_root_schema_for_bundle(
...
void CodecServiceProvider::initialize_root_schema(
    const std::string& bundle_name, path::Repository& repo) {
  ...
  auto root_schema = repo.create_root_schema(
      get_global_capabilities_lookup_tables(), yang_caps);

  std::string s = (user_provided_repo) ? USER_PROVIDED_REPO : bundle_name;
  std::pair<std::string, std::shared_ptr<path::RootSchemaNode>> entry{
      s, root_schema};
