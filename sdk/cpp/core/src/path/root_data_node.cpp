...
//////////////////////////////////////////////////////////////////////////
ydk::path::RootDataImpl::RootDataImpl(const SchemaNode& schema,
                                      struct ly_ctx* ctx,
                                      const std::string& path)
    : DataNodeImpl(nullptr, nullptr, nullptr),
      m_schema(schema),
      m_ctx(ctx),
      m_path(path) {}

ydk::path::RootDataImpl::RootDataImpl(
    const SchemaNode& schema, struct ly_ctx* ctx, const std::string& path,
    const std::shared_ptr<RepositoryPtr>& repo)
    : DataNodeImpl(nullptr, nullptr, repo),
      m_schema(schema),
      m_ctx(ctx),
      m_path(path),
      m_priv_repo(repo) {}
...
const ydk::path::SchemaNode& ydk::path::RootDataImpl::get_schema_node() const {
  return m_schema;
}

std::string ydk::path::RootDataImpl::get_path() const {
  return m_schema.get_path();
}

void ydk::path::RootDataImpl::populate_new_schemas_from_path(
    const std::string& path) {
...
ydk::path::DataNode& ydk::path::RootDataImpl::create_datanode(
  ...
  struct lyd_node* dnode =
      lyd_new_path(m_node, m_ctx, start_seg.c_str(),
                   segments.size() == 1 ? (void*)value.c_str() : nullptr,
                   LYD_ANYDATA_SXML, 0);
  ...
  if (m_node == nullptr) {
    ...
    child_map.insert(std::make_pair(
        m_node, std::make_shared<DataNodeImpl>(this, m_node, m_priv_repo)));
...
void ydk::path::RootDataImpl::set_value(const std::string& value) {
  if (!value.empty()) {
...
std::string ydk::path::RootDataImpl::get_value() const { return ""; }
...
const ydk::path::DataNode& ydk::path::RootDataImpl::get_root() const {
  return *this;
}

std::vector<std::shared_ptr<ydk::path::DataNode>> ydk::path::RootDataImpl::find(
    const std::string& path) {
  populate_new_schemas_from_path(path);
  ...
  const struct lys_node* found_snode = ly_ctx_get_node(
      m_node->schema->module->ctx, nullptr, schema_path.c_str(), 1);
