...
static void get_namespaces_from_xml_doc(
    xmlNodePtr root, std::unordered_set<std::string>& namespaces) {
  xmlNodePtr curr = nullptr;
  for (curr = root; curr; curr = curr->next) {
    if (curr->type == XML_ELEMENT_NODE && curr->ns && curr->ns->href) {
      namespaces.insert(
          std::string{reinterpret_cast<const char*>(curr->ns->href)});
      if (curr->nsDef && curr->nsDef->href) {
        namespaces.insert(
            std::string{reinterpret_cast<const char*>(curr->nsDef->href)});
      }
    }
...
static std::unordered_set<std::string> get_namespaces_from_xml_payload(
...
static void get_module_names_from_json_object(
    json& o, std::unordered_set<std::string>& module_names) {
    ...
    if (it->is_array()) {
      for (auto i = it->begin(); i != it->end(); i++) {
        if (i->is_primitive()) {
          ...
          if (ns.size() > 0) module_names.insert(ns.begin(), ns.end());
        } else {
...
static std::unordered_set<std::string> get_module_names_from_json_payload(
    const std::string& payload) {
...
std::string ydk::path::RootSchemaNode::get_path() const { return "/"; }

ydk::path::SchemaNode* ydk::path::RootSchemaNode::get_parent() const noexcept {
  return nullptr;
}

const ydk::path::SchemaNode& ydk::path::RootSchemaNode::get_root()
    const noexcept {
  return *this;
}

ydk::path::Statement ydk::path::RootSchemaNode::get_statement() const {
  return Statement{};
}

std::vector<ydk::path::Statement> ydk::path::RootSchemaNode::get_keys() const {
...
ydk::path::RootSchemaNodeImpl::RootSchemaNodeImpl(
    struct ly_ctx* ctx, const std::shared_ptr<RepositoryPtr>& repo)
...
void ydk::path::RootSchemaNodeImpl::populate_all_module_schemas() {
  uint32_t idx = 0;
...
void ydk::path::RootSchemaNodeImpl::populate_module_schema(
  ...
  while (auto q = lys_getnext(last, nullptr, module, 0)) {
    m_children.push_back(std::make_unique<SchemaNodeImpl>(
        this, const_cast<struct lys_node*>(q)));
    last = q;
  }
}

void ydk::path::RootSchemaNodeImpl::populate_new_schemas_from_payload(
    const std::string& payload, ydk::EncodingFormat format) {
  ...
  if (format == ydk::EncodingFormat::XML) {
    ...
    modules = m_priv_repo->get_new_ly_modules_from_lookup(
        m_ctx, namespaces, m_name_namespace_lookup);
  } else {
    auto module_names = get_module_names_from_json_payload(payload);
    modules = m_priv_repo->get_new_ly_modules_from_lookup(
        m_ctx, module_names, m_name_namespace_lookup);
  }

  populate_new_schemas(modules);
}

void ydk::path::RootSchemaNodeImpl::populate_new_schemas_from_path(
    const std::string& path) {
  if (path.empty()) return;
  YLOG_DEBUG("Getting new modules for path '{}'", path);
  auto new_modules = m_priv_repo->get_new_ly_modules_from_path(
      m_ctx, path, m_name_namespace_lookup);
  populate_new_schemas(new_modules);
}

void ydk::path::RootSchemaNodeImpl::populate_new_schemas(
...
void ydk::path::RootSchemaNodeImpl::populate_augmented_schema_nodes(
    const struct lys_module* module) {
...
void ydk::path::RootSchemaNodeImpl::populate_augmented_schema_node(
    std::vector<lys_node*>& ancestors, struct lys_node* target) {
  YLOG_DEBUG("Populating augmented schema node '{}'",
  ...
  if (found == false) {
    m_children.push_back(std::make_unique<SchemaNodeImpl>(
        this, const_cast<struct lys_node*>(root)));
  }
  ...
  for (auto& c : m_children) {
    if (c->get_statement().arg == root->name) {
      reinterpret_cast<SchemaNodeImpl*>(c.get())
          ->populate_augmented_schema_node(ancestors, target);
    }
  }
}

std::vector<ydk::path::SchemaNode*> ydk::path::RootSchemaNodeImpl::find(
    const std::string& path) {
  ...
  const struct lys_node* found_node =
      ly_ctx_get_node(m_ctx, nullptr, full_path.c_str(), 0);
...
ydk::path::DataNode& ydk::path::RootSchemaNodeImpl::create_datanode(
    const std::string& path) {
...
ydk::path::DataNode& ydk::path::RootSchemaNodeImpl::create_datanode(
  ...
  auto root_data_node =
      std::make_unique<RootDataImpl>(*this, m_ctx, "/", m_priv_repo);
  m_root_data_nodes.push_back(std::move(root_data_node));
  return m_root_data_nodes.back()->create_datanode(path, value);
}

std::shared_ptr<ydk::path::Rpc> ydk::path::RootSchemaNodeImpl::create_rpc(
    const std::string& path) {
  auto c = find(path);
  if (c.empty()) {
