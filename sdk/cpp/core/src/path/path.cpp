...
static void escape_slashes(std::string& data, const char* pattern) {
  ...
  while (offset < data.size() &&
         (rc = pcre_exec(re, 0, data.c_str(), data.size(), offset, 0, offsets,
                         sizeof(offsets))) >= 0) {
    for (int i = 0; i < rc; ++i) {
  ...
  if (psubStrMatchStr != NULL) pcre_free_substring(psubStrMatchStr);
  free(re);
}
}  // namespace ydk

std::unordered_set<std::string> ydk::path::segmentalize_module_names(
...
void ydk::path::ValidationService::validate(
    const ydk::path::DataNode& dn, ydk::ValidationService::Option option) {
  ...
  YLOG_INFO("Validation called on {} with option {}", dn.get_path(),
            option_str);

  // what kind of a DataNode is this
  const ydk::path::DataNodeImpl& dn_impl =
      dynamic_cast<const ydk::path::DataNodeImpl&>(dn);
  struct lyd_node* lynode = dn_impl.m_node;
  int rc = lyd_validate(&lynode, ly_option, NULL);
  if (rc) {
    YLOG_ERROR("Data validation failed: {}. Path: {}", ly_errmsg(),
               ly_errpath());
...
static ydk::path::RootSchemaNodeImpl& get_root_schema_impl(
    ydk::path::RootSchemaNode& root_schema) {
  ydk::path::RootSchemaNodeImpl& rs_impl =
      dynamic_cast<ydk::path::RootSchemaNodeImpl&>(root_schema);
  return rs_impl;
}

static std::shared_ptr<ydk::path::DataNode> perform_decode(
    ydk::path::RootSchemaNodeImpl& rs_impl, struct lyd_node* lnode) {
  YLOG_DEBUG("Performing decode operation");
  std::shared_ptr<ydk::path::RootDataImpl> rd =
      std::make_shared<ydk::path::RootDataImpl>(rs_impl, rs_impl.m_ctx, "/");
  rd->m_node = lnode;

  struct lyd_node* first_dnode = lyd_first_sibling(lnode);
  struct lyd_node* dnode = first_dnode;
  do {
    rd->child_map.insert(std::make_pair(
        dnode,
        std::make_shared<ydk::path::DataNodeImpl>(rd.get(), dnode, nullptr)));
    dnode = dnode->next;
...
static struct lyd_node* create_lyd_node_for_rpc(
    ydk::path::RootSchemaNodeImpl& rs_impl, const std::string& rpc_path) {
  struct lyd_node* rpc = lyd_new_path(NULL, rs_impl.m_ctx, rpc_path.c_str(),
                                      NULL, LYD_ANYDATA_SXML, 0);
...
std::string ydk::path::Codec::encode(
    std::vector<ydk::path::DataNode*>& data_nodes, ydk::EncodingFormat format,
    bool pretty) {
  YLOG_DEBUG(
      "ydk::path::Codec: Encoding list of data nodes to {} formated string",
      format == ydk::EncodingFormat::JSON ? "JSON" : "XML");
  std::string ret{};
...
std::string ydk::path::Codec::encode(const ydk::path::DataNode& dn,
                                     ydk::EncodingFormat format, bool pretty) {
  YLOG_DEBUG("ydk::path::Codec: Encoding data node '{}' to {} formated string",
             dn.get_path(),
             format == ydk::EncodingFormat::JSON ? "JSON" : "XML");
  std::string ret{};
  if (typeid(dn) == typeid(RootDataImpl)) {
    std::vector<std::shared_ptr<ydk::path::DataNode>> data_nodes =
        dn.get_children();
    for (auto dn : data_nodes) {
      ret += encode(*dn, format, pretty);
    }
    ...
    const ydk::path::DataNodeImpl& impl =
        dynamic_cast<const ydk::path::DataNodeImpl&>(dn);
    ...
    char* buffer = nullptr;
    if (!lyd_print_mem(
            &buffer, m_node, scheme,
            (pretty ? LYP_FORMAT : 0) | LYP_WD_ALL | LYP_KEEPEMPTYCONT)) {
      if (!buffer) {
...
std::shared_ptr<ydk::path::DataNode> ydk::path::Codec::decode(
    RootSchemaNode& root_schema, const std::string& buffer,
  ...
  struct lyd_node* root =
      lyd_parse_mem(rs_impl.m_ctx, buffer.c_str(), get_ly_format(format),
...
std::shared_ptr<ydk::path::DataNode> ydk::path::Codec::decode_rpc_output(
    RootSchemaNode& root_schema, const std::string& buffer,
  ...
  struct lyd_node* root =
      lyd_parse_mem(rs_impl.m_ctx, buffer.c_str(), get_ly_format(format),
                    LYD_OPT_TRUSTED | LYD_OPT_RPCREPLY, rpc, NULL);
  if (root == nullptr || ly_errno) {
    YLOG_ERROR("Parsing failed with message {}", ly_errmsg());
    throw(YCodecError{YCodecError::Error::XML_INVAL});
  }
...
std::shared_ptr<ydk::path::DataNode> ydk::path::Codec::decode_json_output(
  ...
  for (auto buffer : buffer_list) {
    rs_impl.populate_new_schemas_from_payload(buffer,
                                              ydk::EncodingFormat::JSON);

    struct lyd_node* dnode = lyd_parse_mem(
        rs_impl.m_ctx, buffer.c_str(), LYD_JSON, LYD_OPT_TRUSTED | LYD_OPT_GET);
