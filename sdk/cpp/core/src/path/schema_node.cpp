...
// ydk::SchemaNodeImpl
////////////////////////////////////////////////////////////////////
ydk::path::SchemaNodeImpl::SchemaNodeImpl(const SchemaNode* parent,
                                          struct lys_node* node)
    : m_parent{parent}, m_node{node}, m_children{} {
  node->priv = this;
  if (node->nodetype != LYS_LEAF && node->nodetype != LYS_LEAFLIST) {
    ...
    while (auto q = lys_getnext(last, node, nullptr, 0)) {
      m_children.emplace_back(
          make_unique<SchemaNodeImpl>(this, const_cast<struct lys_node*>(q)));
      last = q;
    }
  }
}

void ydk::path::SchemaNodeImpl::populate_augmented_schema_node(
    vector<lys_node*>& ancestors, struct lys_node* node) {
  if (!ancestors.empty()) {
    ...
    for (auto& c : m_children) {
      if (c->get_statement().arg == curr->name) {
        reinterpret_cast<SchemaNodeImpl*>(c.get())
            ->populate_augmented_schema_node(ancestors, node);
      }
    }
  } else {
    const struct lys_node* last = nullptr;
    while (auto p = lys_getnext(last, node->parent, nullptr, 0)) {
      ...
      if (p) {
        ...
        m_children.emplace_back(
            make_unique<SchemaNodeImpl>(this, const_cast<struct lys_node*>(p)));
      }
    }
  }
}
...
string ydk::path::SchemaNodeImpl::get_path() const {
  string ret{};
...
vector<ydk::path::SchemaNode*> ydk::path::SchemaNodeImpl::find(
    const string& path) {
  ...
  const struct lys_node* found_node =
      ly_ctx_get_node(ctx, m_node, path.c_str(), 0);
...
const ydk::path::SchemaNode* ydk::path::SchemaNodeImpl::get_parent()
    const noexcept {
  return m_parent;
}
...
const ydk::path::SchemaNode& ydk::path::SchemaNodeImpl::get_root()
    const noexcept {
  if (m_parent == nullptr) {
    return *this;
  } else {
...
void ydk::path::SchemaNodeImpl::populate_new_schemas_from_path(
    const string& path) {
...
static bool is_submodule(lys_node* node) { return node->module->type; }

ydk::path::Statement ydk::path::SchemaNodeImpl::get_statement() const {
  Statement s{};
  s.arg = m_node->name;
...
///
vector<ydk::path::Statement> ydk::path::SchemaNodeImpl::get_keys() const {
