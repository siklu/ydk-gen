...
namespace ydk {
namespace path {
static void check_ly_schema_node_for_path(lyd_node* node,
                                          const std::string& path) {
  if (node == nullptr || node->schema == nullptr ||
      node->schema->priv == nullptr) {
...
ydk::path::DataNode& ydk::path::DataNode::create_datanode(
    const std::string& path) {
...
ydk::path::DataNodeImpl::DataNodeImpl(
    DataNode* parent, lyd_node* node,
    const std::shared_ptr<RepositoryPtr>& repo)
    : m_parent{parent}, m_node{node}, m_priv_repo{repo} {
  // add the children
  if (m_node && m_node->child &&
    ...
    LY_TREE_FOR(m_node->child, iter) {
      child_map.insert(std::make_pair(
          iter, std::make_shared<DataNodeImpl>(this, iter, m_priv_repo)));
    }
  }
...
const ydk::path::SchemaNode& ydk::path::DataNodeImpl::get_schema_node() const {
  check_ly_schema_node_for_path(m_node, get_path());
  auto schema_ptr = reinterpret_cast<const SchemaNode*>(m_node->schema->priv);
  return *schema_ptr;
}

std::string ydk::path::DataNodeImpl::get_path() const {
...
void ydk::path::DataNodeImpl::populate_new_schemas_from_path(
    const std::string& path) {
...
std::shared_ptr<ydk::path::DataNode> ydk::path::DataNodeImpl::operator()(
    const ydk::path::Session& session) {
  return session.invoke(*this);
}

ydk::path::DataNode& ydk::path::DataNodeImpl::create_action(
    const std::string& path) {
  return create_datanode(path, "");
}

ydk::path::DataNode& ydk::path::DataNodeImpl::create_datanode(
...
ydk::path::DataNode& ydk::path::DataNodeImpl::create_helper(
    const std::string& path, const std::string& value) {
  if (path.empty()) {
  ...
  while (iter != segments.end()) {
    ...
    if (r.empty()) {
      ...
      if (s->nodetype == LYS_LEAFLIST) {
        ...
        throw(YModelError{"Duplicate leaf-list item detected: " +
                          dn->get_path()});
      }
    }
  }
  ...
  for (size_t i = start_index; i < segments.size(); i++) {
    ...
    if (i != segments.size() - 1) {
      YLOG_DEBUG("Creating new data path '{}' in '{}'", child_segment,
                 cn->schema->name);
      cn = lyd_new_path(cn, nullptr, child_segment.c_str(), nullptr,
                        LYD_ANYDATA_SXML, 0);
    } else {
      YLOG_DEBUG("Creating new data path '{}' with value '{}' in '{}'",
                 child_segment, value, cn->schema->name);
      cn = lyd_new_path(cn, nullptr, child_segment.c_str(),
                        (void*)value.c_str(), LYD_ANYDATA_SXML, 0);
    }
    ...
    if (cn == nullptr) {
      ...
      YLOG_ERROR("Could not create data node: {} for path: '{}', value: '{}'",
                 segments[i], path, value);
  ...
  if (first_node_created) {
    dn->child_map.insert(std::make_pair(
        first_node_created,
        std::make_shared<DataNodeImpl>(dn, first_node_created, m_priv_repo)));

    DataNodeImpl* rdn =
        dynamic_cast<DataNodeImpl*>(dn->child_map[first_node_created].get());
...
void ydk::path::DataNodeImpl::set_value(const std::string& value) {
...
std::string ydk::path::DataNodeImpl::get_value() const {
  lys_node* s_node = m_node->schema;
  std::string ret{};
  if (s_node->nodetype == LYS_LEAF || s_node->nodetype == LYS_LEAFLIST) {
    lyd_node_leaf_list* leaf = reinterpret_cast<lyd_node_leaf_list*>(m_node);
...
std::vector<std::shared_ptr<ydk::path::DataNode>> ydk::path::DataNodeImpl::find(
    const std::string& path) {
  populate_new_schemas_from_path(path);
  ...
  const lys_node* found_snode = ly_ctx_get_node(
      m_node->schema->module->ctx, m_node->schema, path.c_str(), 0);
...
ydk::path::DataNode* ydk::path::DataNodeImpl::get_parent() const {
  return m_parent;
}
...
const ydk::path::DataNode& ydk::path::DataNodeImpl::get_root() const {
  if (m_parent) {
...
std::string ydk::path::DataNodeImpl::xml() const {
  std::string ret;
  char* xml = nullptr;
...
void ydk::path::DataNodeImpl::add_annotation(const ydk::path::Annotation& an) {
  ...
  lyd_attr* attr =
      lyd_insert_attr(m_node, nullptr, name.c_str(), an.m_val.c_str());
...
bool ydk::path::DataNodeImpl::remove_annotation(
    const ydk::path::Annotation& an) {
  if (!m_node) {
    return false;
  }
...
std::vector<ydk::path::Annotation> ydk::path::DataNodeImpl::annotations() {
