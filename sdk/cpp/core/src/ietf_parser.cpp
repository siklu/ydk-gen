...
static xmlNodePtr drop_hello_tag(xmlNodePtr root) {
  ...
  while (cur != NULL) {
    if ((!xmlStrcmp(cur->name, to_xmlchar("capabilities")))) return cur;
    cur = cur->next;
  }
  return root;
}
...
//////////////////////////////////////////
IetfCapabilitiesXmlParser::IetfCapabilitiesXmlParser() : doc(NULL) {}
...
vector<string> IetfCapabilitiesXmlParser::parse_yang_1_1(
        ...
        if (xmlStrcmp(c->name, to_xmlchar("modules-state")) == 0) {
          ...
          while (c1 != NULL) {
            if (xmlStrcmp(c1->name, to_xmlchar("module")) == 0) {
              ...
              if (revision.size()) capability << "&revision=" << revision;
              if (features.size()) {
                capability << "&features=";
...
vector<string> IetfCapabilitiesXmlParser::parse(
    const string &capabilities_buffer) {
  vector<string> capabilities{};
  if (doc != NULL) xmlFreeDoc(doc);
  doc = xmlReadMemory(capabilities_buffer.c_str(), capabilities_buffer.size(),
                      "noname.xml", NULL, 0);
  xmlNodePtr cur = check_and_return_root(doc);
  if (cur == NULL) {
    return {};
...
IetfCapabilitiesParser::get_lookup_table(
    vector<path::Capability> &capabilities) const {
...
vector<pair<std::string, path::Capability>>
IetfCapabilitiesParser::segmentalize_capabilities(
  ...
  for (string &c : capabilities) {
    ...
    if (ns_end == string::npos) continue;

    auto c_ns = c.substr(0, ns_end);

    auto module_start = c.find("module=");

    if (module_start == string::npos) continue;

    auto revision_start = c.find("revision=");
    if (revision_start == string::npos) continue;
    ...
    segs.emplace_back(make_pair(
        c_ns,
        path::Capability{c_module, c_revision, c_features, c_deviations}));
  }

  return segs;
}

vector<path::Capability> IetfCapabilitiesParser::parse(
    vector<string> &capabilities) const {
  auto segs = segmentalize_capabilities(capabilities);
  ...
  // add ydk capability
  path::Capability ydk_cap{
      ydk::path::YDK_MODULE_NAME, ydk::path::YDK_MODULE_REVISION, {}, {}};
  ...
  path::Capability ietf_netconf_cap{
      ydk::IETF_NETCONF_MODULE_NAME, ydk::IETF_NETCONF_MODULE_REVISION, {}, {}};
