...
static shared_ptr<path::Rpc> get_rpc_instance(NetconfServiceProvider& provider,
                                              const char* path);
static void create_input_leaf(path::DataNode& input_datanode,
                              DataStore datastore, string&& datastore_string,
                              string& url);
static void create_input_leaf(path::DataNode& input_datanode,
                              DataStore datastore, string&& datastore_string);
...
// cancel_commit
bool NetconfService::cancel_commit(NetconfServiceProvider& provider,
                                   int persist_id) {
  YLOG_INFO("Executing 'cancel-commit' RPC, persist-id={}", persist_id);

  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:cancel-commit");

  if (persist_id > -1) {
    rpc->get_input_node().create_datanode("persist-id",
                                          std::to_string(persist_id));
  }
...
bool NetconfService::close_session(NetconfServiceProvider& provider) {
  ...
  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:close-session");
...
bool NetconfService::commit(NetconfServiceProvider& provider, bool confirmed,
  ...
  if (confirm_timeout > -1) {
    rpc->get_input_node().create_datanode("confirm-timeout",
...
// copy_config
bool NetconfService::copy_config(NetconfServiceProvider& provider,
                                 DataStore target, DataStore source,
                                 string url) {
  YLOG_INFO("Executing 'copy-config' RPC from {} to {}, URL={}",
            datastore_to_string(source), datastore_to_string(target), url);

  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:copy-config");
...
static bool copy_payload(NetconfServiceProvider& provider, DataStore target,
                         string payload) {
  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:copy-config");
...
bool NetconfService::copy_config(NetconfServiceProvider& provider,
                                 DataStore target, Entity& source) {
  YLOG_INFO("Executing 'copy-config' RPC from [{}] to {}",
            source.get_segment_path(), datastore_to_string(target));

  auto top_entity = get_top_entity(&source);
  std::string entity_string =
      (source.ignore_validation && top_entity->is_top_level_class)
...
bool NetconfService::copy_config(NetconfServiceProvider& provider,
                                 DataStore target,
                                 vector<Entity*>& source_list) {
  if (source_list.empty()) {
    YLOG_INFO("NetconfService::copy_config: Source list is empty");
    return false;
  }
  YLOG_INFO("Executing 'copy-config' RPC from {} to {}",
            entity_vector_to_string(source_list), datastore_to_string(target));
...
// delete_config
bool NetconfService::delete_config(NetconfServiceProvider& provider,
                                   DataStore target, std::string url) {
  YLOG_INFO("Executing 'delete-config' RPC, target={}",
            datastore_to_string(target));

  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:delete-config");
...
bool NetconfService::discard_changes(NetconfServiceProvider& provider) {
  ...
  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:discard-changes");
...
// edit_config
static bool edit_payload(NetconfServiceProvider& provider, DataStore target,
                         string& payload, string& default_operation,
                         string& test_option, string& error_option) {
  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:edit-config");
  ...
  if (default_operation.size() > 0) {
    rpc->get_input_node().create_datanode("default-operation",
                                          default_operation);
  }
...
bool NetconfService::edit_config(NetconfServiceProvider& provider,
                                 DataStore target, Entity& config,
                                 string default_operation, string test_option,
                                 string error_option) {
  YLOG_INFO("Executing 'edit-config' RPC on [{}]", config.get_segment_path());

  auto top_entity = get_top_entity(&config);
  std::string payload =
      (config.ignore_validation && top_entity->is_top_level_class)
          ? get_xml_subtree_filter_payload(*top_entity, provider)
          : get_data_payload(*top_entity, provider);

  return edit_payload(provider, target, payload, default_operation, test_option,
                      error_option);
}

bool NetconfService::edit_config(NetconfServiceProvider& provider,
                                 DataStore target, vector<Entity*>& config_list,
                                 string default_operation, string test_option,
                                 string error_option) {
  ...
  YLOG_INFO("Executing 'edit-config' RPC on {}",
            entity_vector_to_string(config_list));
...
static shared_ptr<Entity> xml_codec_payload_to_entity(
    const std::string& payload, shared_ptr<Entity> filter) {
...
static vector<shared_ptr<Entity>> get_from_list(
    NetconfServiceProvider& provider, DataStore source,
    vector<Entity*>& filter_list, const char* path) {
  ...
  for (auto entity : filter_list) {
    ...
    if (top_entity->is_top_level_class) {
      ...
      if (entity->ignore_validation) bypass_validation++;
    } else {
  ...
  if (bypass_validation == filter_list.size()) {
    const path::NetconfSession* ns =
        dynamic_cast<const path::NetconfSession*>(&(provider.get_session()));
    ...
    for (Entity* entity : filter_list) {
      ...
      if (!payload.empty()) {
        auto reply_entity =
            xml_codec_payload_to_entity(payload, top_entity->clone_ptr());
        result_list.push_back(get_child_entity_from_top(reply_entity, *entity));
      } else {
        YLOG_DEBUG(
            "Netconf Service 'get' operation did not return data node on "
            "entity '{}'; returning nullptr.",
            entity_key);
  ...
  if (result_datanode == nullptr) return result_list;
  ...
  for (Entity* entity : filter_list) {
    ...
    if (datanode) {
      ...
      YLOG_DEBUG(
          "Netconf Service 'get' operation did not return data node on entity "
          "'{}'; returning nullptr.",
          internal_key);
      result_list.push_back(nullptr);
    }
...
vector<shared_ptr<Entity>> NetconfService::get(NetconfServiceProvider& provider,
  ...
  return get_from_list(provider, DataStore::na, filter_list,
                       "ietf-netconf:get");
}

vector<shared_ptr<Entity>> NetconfService::get_config(
    NetconfServiceProvider& provider, DataStore source,
    vector<Entity*>& filter_list) {
  if (filter_list.empty()) {
    YLOG_INFO("NetconfService::get: Filter list is empty");
  ...
  YLOG_INFO("Executing 'get-config' RPC on {} from {}",
            entity_vector_to_string(filter_list), datastore_to_string(source));
  return get_from_list(provider, source, filter_list,
                       "ietf-netconf:get-config");
}

static shared_ptr<Entity> get_entity(NetconfServiceProvider& provider,
                                     DataStore source, Entity& filter,
                                     const char* path) {
  // Get the root schema node
  shared_ptr<path::Rpc> rpc = get_rpc_instance(provider, path);
  if (source != DataStore::na) {
  ...
  if (filter.ignore_validation && top_entity->is_top_level_class) {
    ...
    auto reply = ns->execute_netconf_operation(*rpc);
    auto reply_entity =
        xml_codec_payload_to_entity(reply, top_entity->clone_ptr());
    return get_child_entity_from_top(reply_entity, filter);
  }

  shared_ptr<path::DataNode> result_datanode = (*rpc)(provider.get_session());
  if (result_datanode == nullptr) return {};

  vector<shared_ptr<path::DataNode>> data_nodes =
      result_datanode->get_children();
  if (data_nodes.size() == 0) return nullptr;

  return read_datanode(filter, data_nodes[0]);
}

shared_ptr<Entity> NetconfService::get(NetconfServiceProvider& provider,
                                       Entity& filter) {
  YLOG_INFO("Executing 'get' RPC on [{}]", filter.get_segment_path());
  return get_entity(provider, DataStore::na, filter, "ietf-netconf:get");
}

shared_ptr<Entity> NetconfService::get_config(NetconfServiceProvider& provider,
                                              DataStore source,
                                              Entity& filter) {
  YLOG_INFO("Executing 'get' RPC on [{}] from {}", filter.get_segment_path(),
            datastore_to_string(source));
  return get_entity(provider, source, filter, "ietf-netconf:get-config");
}

// kill_session
bool NetconfService::kill_session(NetconfServiceProvider& provider,
                                  unsigned int session_id) {
  YLOG_INFO("Executing 'kill-session' RPC");

  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:kill-session");
...
// unlock
bool NetconfService::unlock(NetconfServiceProvider& provider,
                            DataStore target) {
...
// validate
bool NetconfService::validate(NetconfServiceProvider& provider,
                              DataStore source, string url) {
  YLOG_INFO("Executing 'validate' RPC on {}, URL={}",
            datastore_to_string(source), url);

  // Get the root schema node
  shared_ptr<path::Rpc> rpc =
      get_rpc_instance(provider, "ietf-netconf:validate");
...
static shared_ptr<path::Rpc> get_rpc_instance(NetconfServiceProvider& provider,
  ...
  if (rpc == nullptr) throw(YError{"Unable to create rpc"});
