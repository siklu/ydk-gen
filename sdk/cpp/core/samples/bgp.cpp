...
bool test_bgp_create() {
  ...
  auto& l3vpn_ipv4_unicast = bgp.create_datanode(
      ...
      "");
  l3vpn_ipv4_unicast.create_datanode("config/afi-safi-name",
  ...
  auto& neighbor_af = neighbor.create_datanode(
      ...
      "afi-safi[afi-safi-name='openconfig-bgp-types:L3VPN_IPV4_UNICAST']",
      "");
  neighbor_af.create_datanode("config/afi-safi-name",
                              "openconfig-bgp-types:L3VPN_IPV4_UNICAST");
...
bool test_bgp_read() {
  ...
  if (!dn) return false;
  // auto read_xml = s.encode(*(dn->get_children()[0]),
  // ydk::EncodingFormat::XML, true); std::cout << read_xml << std::endl;
  return true;
}
...
int main() {
  ...
  if (!test_bgp_create() || !test_bgp_read() || !test_bgp_delete()) return 1;
