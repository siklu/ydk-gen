...
TEST_CASE(bgp_read) {
  ...
  CHECK_EQUAL(bgp_set->global_->config->as_,
              bgp_read_ptr->global_->config->as_);
  CHECK_EQUAL(bgp_set->neighbors->neighbor[0]->neighbor_address,
              bgp_read_ptr->neighbors->neighbor[0]->neighbor_address);
  CHECK_EQUAL(bgp_set->neighbors->neighbor[0]->config->local_as,
              bgp_read_ptr->neighbors->neighbor[0]->config->local_as);
  CHECK_EQUAL(bgp_set->global_->afi_safis->afi_safi[0]->afi_safi_name,
              bgp_read_ptr->global_->afi_safis->afi_safi[0]->afi_safi_name);
  CHECK_EQUAL(
      bgp_set->global_->afi_safis->afi_safi[0]->config->afi_safi_name,
      bgp_read_ptr->global_->afi_safis->afi_safi[0]->config->afi_safi_name);

  //  cerr<<bgp_set->global_->afi_safis->afi_safi[0]->config->enabled<<","<<bgp_read_ptr->global_->afi_safis->afi_safi[0]->config->enabled<<endl;
  //  res &= string(bgp_set->global_->afi_safis->afi_safi[0]->config->enabled)
  //  == string(bgp_read_ptr->global_->afi_safis->afi_safi[0]->config->enabled);
  //  REQUIRE(res==1);
}
