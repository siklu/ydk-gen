...
void config_isis(Isis* isis) {
  ...
  af->af_data = make_unique<
      Isis::Instances::Instance::Afs::Af::AfData>();  // instantiate the
                                                      // presence node
  af->af_data->parent = af.get();                     // set the parent
  auto metric_style = make_unique<
  ...
  // loopback interface
  auto interface =
      make_unique<Isis::Instances::Instance::Interfaces::Interface>();
  interface->interface_name = "Loopback0";
  interface->running = Empty();
  interface->state = IsisInterfaceState::passive;
  // interface address family
  auto interface_af = make_unique<Isis::Instances::Instance::Interfaces::
                                      Interface::InterfaceAfs::InterfaceAf>();
  ...
  interface_af = make_unique<Isis::Instances::Instance::Interfaces::Interface::
                                 InterfaceAfs::InterfaceAf>();
  interface_af->af_name = IsisAddressFamily::ipv4;
  interface_af->saf_name = IsisSubAddressFamily::unicast;
  interface_af->interface_af_data->running = Empty();
  interface_af->parent = interface->interface_afs.get();
