...
int main(int argc, char* argv[]) {
  ...
  cout << "Router ID: " << bgp_read_ptr->global->config->router_id << endl
       << endl;

  for (size_t index = 0; index < bgp_read_ptr->neighbors->neighbor.size();
       index++) {
    openconfig_bgp::Bgp::Neighbors::Neighbor& neighbor =
        *(bgp_read_ptr->neighbors->neighbor[index]);
    ...
    cout << "Neighbor peer type: " << neighbor.config->peer_type << endl
         << endl;
  }

  for (size_t index = 0;
       index < bgp_read_ptr->global->afi_safis->afi_safi.size(); index++) {
    openconfig_bgp::Bgp::Global::AfiSafis::AfiSafi& afi_safi =
        *(bgp_read_ptr->global->afi_safis->afi_safi[index]);
  ...
  for (size_t index = 0; index < bgp_read_ptr->peer_groups->peer_group.size();
       index++) {
    openconfig_bgp::Bgp::PeerGroups::PeerGroup& peer_group =
        *(bgp_read_ptr->peer_groups->peer_group[index]);
