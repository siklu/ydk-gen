...
static void config_bgp(Bgp* bgp) {
  ...
  auto global_af = make_unique<Bgp::Instance::InstanceAs::FourByteAs::
                                   DefaultVrf::Global::GlobalAfs::GlobalAf>();
  ...
  four_byte_as->default_vrf->global->global_afs->global_af.push_back(
      move(global_af));

  // configure IBGP neighbor group
  auto neighbor_group =
      make_unique<Bgp::Instance::InstanceAs::FourByteAs::DefaultVrf::BgpEntity::
  ...
  // ipv4 unicast
  auto neighbor_group_af = make_unique<
      Bgp::Instance::InstanceAs::FourByteAs::DefaultVrf::BgpEntity::
          NeighborGroups::NeighborGroup::NeighborGroupAfs::NeighborGroupAf>();
  ...
  neighbor_group->parent =
      four_byte_as->default_vrf->bgp_entity->neighbor_groups.get();
  neighbor_group->neighbor_group_afs->neighbor_group_af.push_back(
      move(neighbor_group_af));
  four_byte_as->default_vrf->bgp_entity->neighbor_groups->neighbor_group
      .push_back(move(neighbor_group));

  // configure IBGP neighbor
  auto neighbor = make_unique<Bgp::Instance::InstanceAs::FourByteAs::
                                  DefaultVrf::BgpEntity::Neighbors::Neighbor>();
  neighbor->neighbor_address = "172.16.255.2";
  neighbor->neighbor_group_add_member = "IBGP";
  neighbor->parent = four_byte_as->default_vrf->bgp_entity->neighbors.get();
  four_byte_as->default_vrf->bgp_entity->neighbors->neighbor.push_back(
      move(neighbor));
...
int main(int argc, char* argv[]) {
  ...
  try {
    ydk::path::Repository repo{
        "/Users/abhirame/Cisco/odl/distribution-karaf-0.5.2-Boron-SR2/cache/"
        "schema"};
    OpenDaylightServiceProvider provider{repo,     host, username,
                                         password, port, EncodingFormat::XML};
    CrudService crud{};
