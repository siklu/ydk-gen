...
int main(int argc, char* argv[]) {
  ...
  for (size_t i = 0; i < isis_read_ptr->instances->instance.size(); i++) {
    auto instance = dynamic_cast<Isis::Instances::Instance*>(
        isis_read_ptr->instances->instance[i].get());
    ...
    for (size_t j = 0; j < instance->interfaces->interface.size(); j++) {
      cout << endl
           << "Interface name: "
           << instance->interfaces->interface[j]->interface_name << endl;
      cout << "Interface running: "
           << ((instance->interfaces->interface[j]->running.is_set) ? "Yes"
                                                                    : "No")
           << endl;
      cout << "Interface state: " << instance->interfaces->interface[j]->state
           << endl;
      cout << "Interface point-to-point: "
           << ((instance->interfaces->interface[j]->point_to_point.is_set)
                   ? "Yes"
                   : "No")
           << endl;

      for (size_t k = 0; k < instance->interfaces->interface[j]
                                 ->interface_afs->interface_af.size();
           k++) {
        cout << "Interface AF name: "
             << instance->interfaces->interface[j]
                    ->interface_afs->interface_af[k]
                    ->af_name
             << endl;
        cout << "Interface SAF name: "
             << instance->interfaces->interface[j]
                    ->interface_afs->interface_af[k]
                    ->saf_name
             << endl;
