...
TEST_CASE("bgp_read_delete") {
  ...
  openconfig_bgp::Bgp* bgp_read_ptr =
      dynamic_cast<openconfig_bgp::Bgp*>(bgp_read.get());
  REQUIRE(bgp_read_ptr != nullptr);

  REQUIRE(*(bgp_read_ptr) == *(bgp_set));

  CHECK(bgp_set->global->config->as == bgp_read_ptr->global->config->as);
  auto set_neighbor = dynamic_cast<openconfig_bgp::Bgp::Neighbors::Neighbor*>(
      bgp_set->neighbors->neighbor[0].get());
  auto read_neighbor = dynamic_cast<openconfig_bgp::Bgp::Neighbors::Neighbor*>(
      bgp_read_ptr->neighbors->neighbor[0].get());
  ...
  auto set_afi_safi =
      dynamic_cast<openconfig_bgp::Bgp::Global::AfiSafis::AfiSafi*>(
          bgp_set->global->afi_safis->afi_safi[0].get());
  auto read_afi_safi =
      dynamic_cast<openconfig_bgp::Bgp::Global::AfiSafis::AfiSafi*>(
          bgp_read_ptr->global->afi_safis->afi_safi[0].get());
  CHECK(set_afi_safi->afi_safi_name == read_afi_safi->afi_safi_name);
  CHECK(set_afi_safi->config->afi_safi_name ==
        read_afi_safi->config->afi_safi_name);

  reply = reply &&
          (set_afi_safi->config->enabled == read_afi_safi->config->enabled);
  REQUIRE(reply);
...
TEST_CASE("oc_platform") {
  ...
  openconfig_platform::Components* comp_r =
      dynamic_cast<openconfig_platform::Components*>(data.get());
  REQUIRE(comp_r != nullptr);

  auto read_comp = dynamic_cast<openconfig_platform::Components::Component*>(
      comp_r->component[0].get());
...
TEST_CASE("crud_read_non_top_container") {
  ...
  auto address =
      make_shared<ydktest_sanity::Native::Interface::Loopback::Ipv4::Address>();
  address->ip = "2.2.2.2";
  ...
  auto ipv4 = dynamic_cast<ydktest_sanity::Native::Interface::Loopback::Ipv4*>(
      read_entity_wrapper.get());
  auto ip_address =
      dynamic_cast<ydktest_sanity::Native::Interface::Loopback::Ipv4::Address*>(
          ipv4->address["2.2.2.2"].get());
