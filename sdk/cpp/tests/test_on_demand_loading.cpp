...
std::string AUGMENTED_XML_PAYLOAD =
    R"(<cpython xmlns="http://cisco.com/ns/yang/ietf-aug-base-1">
  <doc>
...
TEST_CASE("on_demand_loading_json") {
  ...
  auto cpython = codec.decode(root_schema, AUGMENTED_JSON_PAYLOAD,
...
TEST_CASE("on_demand_loading_xml") {
  ...
  auto cpython = codec.decode(root_schema, AUGMENTED_XML_PAYLOAD,
                              ydk::EncodingFormat::XML);
  REQUIRE(cpython);
}
