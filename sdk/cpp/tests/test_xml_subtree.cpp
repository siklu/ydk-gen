...
TEST_CASE("subtree_encode_leaf") {
  ...
  ydk::NetconfServiceProvider provider{repo, "127.0.0.1", "admin", "admin",
                                       12022};
...
TEST_CASE("subtree_encode_namespaces") {
  ...
  REQUIRE(s ==
          R"(<routing-policy xmlns="http://openconfig.net/yang/routing-policy">
  <policy-definitions>
    <policy-definition>
      <name>xyz</name>
      <statements>
...
TEST_CASE("subtree_decode_crud_read") {
  ...
  auto s =
      codec.decode(R"(<runner xmlns="http://cisco.com/ns/yang/ydktest-sanity">
<ytypes>
    <built-in-t>
    <number8>102</number8>
    </built-in-t>
...
<ytypes>
  ...
  REQUIRE(((ydktest_sanity::Runner&)*s).ytypes->built_in_t->number8 ==
...
TEST_CASE("test_no_key_list") {
  ...
  auto no_key =
      xml_codec.decode(payload, make_shared<ydktest_sanity::Runner>());
...
TEST_CASE("xml_codec_augment_presence") {
  ...
  auto cs_runner = codec_service.decode(codec_provider, xmlC,
