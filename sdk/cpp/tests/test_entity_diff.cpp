...
void print_dictionary(const string& legend, map<string, string>& ent_dict) {
  ...
  for (map<string, string>::iterator it = ent_dict.begin();
       it != ent_dict.end(); ++it) {
    cout << it->first << ": " << check_empty_str_value(it->second) << endl;
  }
}

void print_diffs(map<string, pair<string, string>>& diff, Entity& ent_left,
                 Entity& ent_right) {
  cout << "\n------> DIFFS:" << endl;
  for (auto const& entry : diff) {
    auto value_pair = entry.second;
    cout << entry.first << ": " << check_empty_str_value(value_pair.first)
         << " vs " << check_empty_str_value(value_pair.second) << endl;
...
TEST_CASE("test_entity_to_dict_enum_leaflist") {
  auto runner = ydktest_sanity::Runner{};
  runner.ytypes->built_in_t->enum_llist.append(
...
TEST_CASE("test_entity_diff_no_keys") {
  ...
  auto r_2 = ydktest_sanity::Runner();
  t1 = make_shared<ydktest_sanity::Runner::NoKeyList>();
  t1->test = "t1";
  t2 = make_shared<ydktest_sanity::Runner::NoKeyList>();
  t2->test = "tt";
