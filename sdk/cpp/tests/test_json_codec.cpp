...
TEST_CASE("json_codec_leaf") {
  ...
  REQUIRE(
      json ==
      R"({"ydktest-sanity:runner":{"ytypes":{"built-in-t":{"number16":102,"number8":10}}}})");
...
TEST_CASE("json_codec_leaf_list") {
  ...
  REQUIRE(
      json ==
      R"({"ydktest-sanity:runner":{"ytypes":{"built-in-t":{"llstring":["abc","klm","xyz"]}}}})");
...
TEST_CASE("json_codec_identity_subtree") {
  ...
  auto top_entity =
      make_shared<openconfig_bgp::Bgp::Global::AfiSafis::AfiSafi>();
  auto entity = jcodec.decode(json, top_entity);
  auto afi_safi_d =
...
TEST_CASE("json_codec_no_key_list") {
  ...
  REQUIRE(
      json ==
      R"({"ydktest-sanity:runner":{"no-key-list":[{"test":"abc"},{"test":"xyz"}]}})");
...
TEST_CASE("json_codec_augment_presence") {
  ...
  auto cs_runner = codec_service.decode(codec_provider, jsonC,
