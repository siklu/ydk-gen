...
#define CONTAINS_ERROR_MESSAGE                                                \
  Catch::Contains("Invalid value") || Catch::Contains("Failed to resolve") || \
      ...
      Catch::Contains("got unexpected data") || Catch::Contains("YModelError")
...
TEST_CASE("int8_invalid_ignore_validation") {
  ...
  CHECK_THROWS_WITH(crud.update(provider, r),
                    Catch::Contains("\"test\" is not a valid value"));
}
...
TEST_CASE("leaflist_max_elements") {
  ...
  r_1->ytypes->built_in_t->enum_llist.append(
      ydktest_sanity::YdkEnumTest::not_set);
  r_1->ytypes->built_in_t->enum_llist.append(
...
TEST_CASE("leaflist_duplicate") {
  ...
  r_1->ytypes->built_in_t->enum_llist.append(ydktest_sanity::YdkEnumTest::none);
  r_1->ytypes->built_in_t->enum_llist.append(ydktest_sanity::YdkEnumTest::none);
  CHECK_THROWS_WITH(crud.create(provider, *r_1),
                    CONTAINS_ERROR_MESSAGE);  // TODO
}
...
TEST_CASE("int8_valid_ignore_validation") {
  ...
  REQUIRE(runner->ytypes->built_in_t->number8 ==
          r_1->ytypes->built_in_t->number8);
  ...
  auto bint =
      dynamic_cast<ydktest_sanity::Runner::Ytypes::BuiltInT*>(bint_ent.get());
