...
//    NetconfServiceProvider provider{repo, "127.0.0.1", "admin", "admin",
//    12022}; NetconfService ns{};
//
...
TEST_CASE("get_edit_copy_config") {
  ...
  if (ns.edit_config(provider, target, config_list)) ns.commit(provider);
...
TEST_CASE("iana_if_type_filter") {
  ...
  auto ifc_xml =
      entity2string(ifc_ent, provider.get_session().get_root_schema());
...
TEST_CASE("edit_multiple_config") {
  ...
  for (auto item : read_list) {
    ...
    if (path.find("bgp") != string::npos) {
      ...
      REQUIRE(data_ptr != nullptr);
      REQUIRE(data_ptr->global->config->as == bgp.global->config->as);
      REQUIRE(data_ptr->global->config->router_id ==
              bgp.global->config->router_id);
    }
    if (path.find("native") != string::npos) {
      auto data_ptr =
          dynamic_cast<ydktest::ydktest_sanity::Native*>(item.get());
      REQUIRE(data_ptr != nullptr);
...
TEST_CASE("ietf_get_config_rpc") {
  ...
  std::shared_ptr<ydk::path::Rpc> read_rpc{
  ...
  // Print config
  vector<shared_ptr<ydk::path::DataNode>> data_nodes =
      read_result->get_children();
...
TEST_CASE("ietf_get_rpc") {
  ...
  std::shared_ptr<ydk::path::Rpc> read_rpc{
      schema.create_rpc("ietf-netconf:get")};
  try {
...
TEST_CASE("create_get_non_top") {
  ...
  auto neighbor_filter =
      make_shared<openconfig_bgp::Bgp::Neighbors::Neighbor>();
  neighbor_filter->neighbor_address = "1.2.3.4";
  bgp_filter.neighbors->neighbor.append(neighbor_filter);
  auto neighbor_entity =
      ns.get_config(provider, DataStore::candidate, *neighbor_filter);
  REQUIRE(neighbor_entity != nullptr);

  auto neighbor_ptr = dynamic_cast<openconfig_bgp::Bgp::Neighbors::Neighbor*>(
      neighbor_entity.get());
...
TEST_CASE("get_openconfig_interfaces_and_bgp") {
  ...
  interfaces.create_datanode(
      "interface[name='GigabitEthernet0/0/0/2']/config/name",
      "GigabitEthernet0/0/0/2");
  interfaces.create_datanode(
      "interface[name='GigabitEthernet0/0/0/2']/config/type",
  ...
  std::shared_ptr<ydk::path::Rpc> edit_rpc{
      schema.create_rpc("ietf-netconf:edit-config")};
...
namespace ydk {
namespace path {
std::shared_ptr<DataNode> handle_rpc_output(const std::string& reply,
...
exit
  ...
  auto reply_dn = ydk::path::handle_rpc_output(
      rpc_reply, schema, rpc->get_input_node().get_path());
