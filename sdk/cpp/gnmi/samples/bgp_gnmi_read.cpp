...
int main(int argc, char* argv[]) {
  ...
  //    cout << "Router ID: " << bgp_read_ptr->global->config->router_id <<
  //    endl<<endl;

  cout << endl << "BGP neighbors: " << endl;
  for (auto neighbor_entity : bgp_read_ptr->neighbors->neighbor.entities()) {
    auto neighbor = dynamic_cast<openconfig_bgp::Bgp::Neighbors::Neighbor*>(
        neighbor_entity.get());
    cout << "  Neighbor address: " << neighbor->neighbor_address << endl;
    //        cout << "Neighbor local AS: " <<  neighbor.config->local_as <<
    //        endl; cout << "  Neighbor peer group: " <<
    //        neighbor->config->peer_group << endl;
    cout << "  Neighbor peer AS: " << neighbor->config->peer_as << endl << endl;
  }

  //    for(size_t index=0; index <
  //    bgp_read_ptr->global->afi_safis->afi_safi.size(); index++)
  //    {
  //        openconfig::openconfig_bgp::Bgp::Global::AfiSafis::AfiSafi &
  //        afi_safi = *(bgp_read_ptr->global->afi_safis->afi_safi[index]); cout
  //        << "AFI-SAFI name: " << afi_safi.afi_safi_name <<endl; cout <<
  //        "AFI-SAFI config name: " <<  afi_safi.config->afi_safi_name <<endl;
  //        cout << "AFI-SAFI enabled: " <<  afi_safi.config->enabled
  //        <<endl<<endl;
  //    }
