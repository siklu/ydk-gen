...
void show_usage(string name) {
  cerr << "\nUsage:\n\t" << name
       << " [http|ssh]://user:password@host[:port] [-v] [-m mode]" << endl;
  cerr << "\t\tIf host is IPv6, enclose its address in square brackets. Ex.: "
          "[1234:1234:1234:1234::1234]"
       << endl;
  cerr << "\t\tIf port is not specified, 57400 will be used to connect to gNMI "
          "server"
       << endl;
  cerr << "\t\tOptions:" << endl;
  cerr << "\t\t\t-v       output debugging information" << endl;
  cerr << "\t\t\t-m mode  subscribe mode, one of 'ONCE', 'POLL', or 'STREAM', "
          "default - 'ONCE'"
       << endl;
  cerr << endl;
}
  ...
  for (int i = 1; i < argc; i++) {
    try {
      ...
      if (arg.find("://") != string::npos) {
        ...
        cerr << "Unexpected argument '" << arg << "' in the command line"
             << endl;
      }
    } catch (...) {
      show_usage(argv[0]);
...
void print_tree(ydk::path::DataNode* dn, const std::string& indent) {
  try {
    ...
    if (s.keyword == "leaf" || s.keyword == "leaf-list" ||
      ...
      std::cout << indent << "<" << s.arg << ">" << val << "</" << s.arg << ">"
                << std::endl;
    } else {
      std::string child_indent{indent};
      child_indent += "  ";
      std::cout << indent << "<" << s.arg << ">" << std::endl;
      for (auto c : dn->get_children()) print_tree(c.get(), child_indent);
      std::cout << indent << "</" << s.arg << ">" << std::endl;
    }
