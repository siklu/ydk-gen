...
// static RootSchemaNodeWrapper* wrap(shared_ptr<ydk::path::RootSchemaNode>
// node)
//{
...
ServiceProvider GnmiServiceProviderInit(YDKStatePtr state, Repository repo,
  ...
  try {
    ...
    ydk::gNMIServiceProvider* real_provider =
        new ydk::gNMIServiceProvider(*real_repo, address, port, username,
                                     password, server_certificate, private_key);
...
void GnmiServiceProviderFree(ServiceProvider provider) {
  ydk::gNMIServiceProvider* real_provider =
      static_cast<ydk::gNMIServiceProvider*>(provider);
  if (real_provider != NULL) {
    delete real_provider;
  }
}

RootSchemaWrapper GnmiServiceProviderGetRootSchemaNode(
    YDKStatePtr state, ServiceProvider provider) {
  try {
...
GnmiSession GnmiServiceProviderGetSession(YDKStatePtr state,
...
DataNode GnmiServiceGetFromPath(YDKStatePtr state, ServiceProvider provider,
                                GnmiPathList path_list, const char* operation) {
  try {
    ydk::gNMIServiceProvider* real_provider =
        static_cast<ydk::gNMIServiceProvider*>(provider);
    ...
    shared_ptr<ydk::path::DataNode> real_datanode =
        gs.get_from_path(*real_provider, *real_list, oper);
...
const char* GnmiServiceGetCapabilities(YDKStatePtr state,
                                       ServiceProvider provider) {
  try {
    ydk::gNMIServiceProvider* real_provider =
        static_cast<ydk::gNMIServiceProvider*>(provider);
...
GnmiSession GnmiSessionInit(YDKStatePtr state, Repository repo,
  ...
  try {
    ...
    ydk::path::gNMISession* real_session =
        new ydk::path::gNMISession(*real_repo, address, port, username,
                                   password, server_certificate, private_key);
...
void GnmiSessionFree(GnmiSession session) {
  ydk::path::gNMISession* real_session =
      static_cast<ydk::path::gNMISession*>(session);
  if (real_session != NULL) {
    delete real_session;
  }
}

DataNode GnmiSessionExecuteRpc(YDKStatePtr state, GnmiSession session,
                               Rpc rpc) {
  try {
    ydk::path::gNMISession* real_session =
        static_cast<ydk::path::gNMISession*>(session);
...
RootSchemaWrapper GnmiSessionGetRootSchemaNode(YDKStatePtr state,
                                               GnmiSession session) {
  try {
...
const char* GetLastSubscribeResponse(YDKStatePtr state, GnmiSession session,
  ...
  int counter = 0;
  while (++counter < 10 &&
         (last_subscribe_response.length() == 0 ||
          last_subscribe_response == previous_subscribe_response)) {
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    if (!GnmiSessionSubscribeInProgress(state, session)) break;
  }
  if (counter < 10) {
...
boolean GnmiSessionSubscribeInProgress(YDKStatePtr state, GnmiSession session) {
  try {
    ydk::path::gNMISession* gnmi_session =
        static_cast<ydk::path::gNMISession*>(session);
...
void GnmiPathListFree(GnmiPathList list) {
  auto real_list = static_cast<GnmiPathVector*>(list);
  for (gnmi::Path* path : *real_list) free(path);
  real_list->clear();
  free(real_list);
}
