...
gNMIServiceProvider::gNMIServiceProvider(path::Repository& repo,
    ...
    : session{repo,       address, port, username, password, server_certificate,
              private_key} {
  string secure = (server_certificate.length() > 0) ? "Secure" : "Insecure";
  YLOG_INFO("gNMIServiceProvider Connected to {} via {} Channel", address,
            secure);
}
...
shared_ptr<Entity> gNMIServiceProvider::execute_operation(
    const string& operation, Entity& entity, map<string, string> params) {
  gNMIService gs;
  if (operation == "create" || operation == "update" || operation == "delete") {
    if (entity.yfilter == YFilter::not_set) {
      entity.yfilter =
          (operation == "delete") ? YFilter::delete_ : YFilter::update;
    }
    gs.set(*this, entity);
  } else if (operation == "read") {
    string read_mode = "ALL";
    if (params["mode"] == "config") read_mode = "CONFIG";
    return gs.get(*this, entity, read_mode);
  } else {
    YLOG_ERROR(
        "gNMIServiceProvider::execute_operation: Operation '{}' is not "
        "supported",
        operation);
    throw(YServiceProviderError(
        "gNMIServiceProvider::execute_operation: Operation is not supported"));
  }
  return nullptr;
}

vector<shared_ptr<Entity>> gNMIServiceProvider::execute_operation(
