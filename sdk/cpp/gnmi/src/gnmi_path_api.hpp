...
class gNMISession : public Session {
 public:
  gNMISession(Repository& repo, const std::string& address, int port,
  ...
  void invoke_subscribe(
      Rpc& rpc, std::function<void(const char* response)> out_func = nullptr,
  ...
  std::shared_ptr<path::DataNode> handle_get_reply(
      std::vector<std::string> reply_val) const;
 ...
 private:
  ...
  void handle_subscribe(
      path::Rpc& ydk_rpc, std::function<void(const char* response)> out_func,
