...
static std::shared_ptr<Channel> connect_to_server(
    const std::string& address, int port, const std::string& server_certificate,
  ...
  server_cert.assign((istreambuf_iterator<char>(p)),
  ...
  if (private_key.length() > 0) {
    ...
    client_key.assign((istreambuf_iterator<char>(k)),
                      (istreambuf_iterator<char>()));
...
static void populate_subscribe_request(gnmi::SubscriptionList* sl,
                                       GnmiClientSubscription subscription) {
...
static string format_notification_response(pair<string, string> prefix_suffix,
                                           const std::string& value) {
  string reply_val;
  string path_to_prepend = prefix_suffix.first;
  string path_to_append = prefix_suffix.second;

  // TODO: Update again when payload format from the server(IOS XR) is made
  // consistent with different request paths
  //    if (flag.path_has_value)
  //        reply_val = "{" + path_to_prepend + "{[" + value + "]}}";
  //    else if (flag.prefix_has_value)
  //        reply_to_parse.append("\"data\":{" + prefix_to_prepend + ":[" +
...
void gNMIClient::parse_capabilities_modeldata(
  ...
  for (size_t i = 0, n = response->supported_models_size(); i < n; i++) {
    ...
    if (!(modeldata.organization().empty())) os << organization << ", ";
  ...
  if (update.has_path()) {
    ...
    if (elem_size > 0) {
      ...
      for (l = 0; l < elem_size; l++) {
        ...
        if (l == elem_size - 1 && value.length() > 0) break;
        ...
        if (path.elem(l).key_size() == 0) {
          ...
          for (auto key : path.elem(l).key()) {
            ...
            bool is_number =
                all_of(key.second.begin(), key.second.end(), ::isdigit);
            if ((ckey[0] == '\"' && ckey[key.second.length() - 1] == '\"') ||
                is_number) {
              // The key value is a number or string, which is already
              // surrounded by double-quotes like in implementation of XR gNMI
              // server
              keys_to_append.append("\"" + key.first + "\":" + key.second +
                                    ",");
            } else {
              // The key value is not a number, treat it as a string by adding
              // double-quotes
              keys_to_append.append("\"" + key.first + "\":\"" + key.second +
                                    "\",");
            }
      ...
      if (value.length() == 0) {
        if (path_to_prepend[path_to_prepend.length() - 1] == ',') {
          path_to_prepend =
              path_to_prepend.substr(0, path_to_prepend.length() - 1);
        }
      }
    }
  } else {
...
vector<string> gNMIClient::execute_get_operation(
    const std::vector<GnmiClientRequest>& get_request_list,
  ...
  for (auto ydk_request : get_request_list) {
    ...
    if (ydk_request.path != nullptr) path->CopyFrom(*ydk_request.path);
  }
...
bool gNMIClient::execute_set_operation(
    const std::vector<ydk::GnmiClientRequest>& set_request_list) {
  ...
  for (auto request : set_request_list) {
    if (request.operation == "delete") {
      ...
      if (request.path != nullptr) path->CopyFrom(*request.path);
    }
  }
  // replace
  for (auto request : set_request_list) {
...
vector<string> gNMIClient::execute_get_payload(const GetRequest& request,
...
bool gNMIClient::execute_set_payload(const SetRequest& request,
...
void poll_thread_callback_control(
    gNMIClient* client, std::function<bool(const char* response)> poll_func) {
  YLOG_DEBUG(
      "Invoking polling/streaming thread control from user defined callback");
  ...
  std::thread writer([client, poll_func]() {
    ...
    while (poll_func(str_response.c_str())) {
      if (!client->client_reader_is_active) {
        YLOG_DEBUG(
            "Client subscription reader is not active. Ending "
            "Polling/Streaming control thread");
        break;
      }
      client->send_poll_request();
      ...
      int counter = 0;
      while (client->get_last_subscribe_response() == str_response &&
             ++counter < 100) {
      ...
      if (counter >= 100) {
        ...
        YLOG_DEBUG("Received subscribe response in {} milliseconds",
                   counter * 100);
...
void gNMIClient::execute_subscribe_operation(
    const std::vector<GnmiClientSubscription>& subscription_list, uint32 qos,
    const std::string& list_mode, const std::string& encoding,
  ...
  while (client_reader_writer->Read(&response)) {
    ...
    google::protobuf::TextFormat::PrintToString(response,
                                                &last_subscribe_response);
    if (out_func != nullptr) {
