...
gNMISession::gNMISession(Repository& repo, const std::string& address, int port,
                         const std::string& username,
                         const std::string& password,
                         const std::string& server_certificate,
                         const std::string& private_key) {
  // Correct default settings
  if (port == 0) port = 57400;

  client = make_unique<gNMIClient>(address, port, username, password,
...
RootSchemaNode& gNMISession::get_root_schema() const { return *root_schema; }

static SchemaNode* get_schema_for_operation(RootSchemaNode& root_schema,
                                            string operation) {
...
void gNMISession::invoke_subscribe(
    Rpc& rpc, std::function<void(const char* response)> out_func,
    std::function<bool(const char* response)> poll_func) const {
  SchemaNode* gnmi_sub =
      get_schema_for_operation(*root_schema, "ydk:gnmi-subscribe");
...
shared_ptr<DataNode> gNMISession::invoke(Rpc& rpc) const {
  ...
  SchemaNode* gnmi_cap =
      get_schema_for_operation(*root_schema, "ydk:gnmi-caps");
...
shared_ptr<DataNode> gNMISession::invoke(DataNode& datanode) const {
  throw(YOperationNotSupportedError{
      "gNMISession::invoke: action datanode is not supported!"});
  return nullptr;
}

static void populate_path_from_payload(gnmi::Path* path, const string& payload,
...
static GnmiClientRequest build_set_request(RootSchemaNode& root_schema,
                                           DataNode* request,
  ...
  if (operation == "delete") {
    populate_path_from_payload(one_request.path, one_request.payload,
                               root_schema);
  } else {
    ...
    if (pos != string::npos) {
      ...
      one_request.payload = one_request.payload.substr(
          pos, one_request.payload.length() - pos - 1);
    }
  }
  return one_request;
}
...
bool gNMISession::handle_set(Rpc& ydk_rpc) const {
  ...
  if (!delete_list.empty()) {
    for (auto request : delete_list) {
      GnmiClientRequest one_request =
          build_set_request(get_root_schema(), request.get(), "delete");
  ...
  if (!replace_list.empty()) {
    for (auto request : replace_list) {
      GnmiClientRequest one_request =
          build_set_request(get_root_schema(), request.get(), "replace");
      setRequest.push_back(one_request);
    }
  }
  ...
  if (!update_list.empty()) {
    for (auto request : update_list) {
      GnmiClientRequest one_request =
          build_set_request(get_root_schema(), request.get(), "update");
      setRequest.push_back(one_request);
    }
  }
...
shared_ptr<DataNode> gNMISession::handle_get(Rpc& rpc) const {
...
shared_ptr<DataNode> gNMISession::handle_get_reply(
    vector<string> reply_val) const {
  Codec codec{};

  shared_ptr<DataNode> root_dn =
      codec.decode_json_output(get_root_schema(), reply_val);
  if (!root_dn) {
...
shared_ptr<DataNode> gNMISession::handle_get_capabilities() const {
...
void gNMISession::handle_subscribe(
    Rpc& rpc, std::function<void(const char* response)> out_func,
    std::function<bool(const char* response)> poll_func) const {
  ...
  if (subscription.empty()) {
    YLOG_ERROR("Failed to get 'subscription' node from '{}' RPC",
               rpc.get_name());
    throw(YInvalidArgumentError{"Failed to get 'request' node from RPC"});
  }
  uint32 qos = 0;
  ...
  if (subscription_list.empty()) {
    YLOG_ERROR("Failed to get 'subscription-list' node from '{}' RPC",
               rpc.get_name());
    throw(YInvalidArgumentError{"Failed to get 'request' node from RPC"});
  }
  ...
  for (auto one_subscription : subscription_list) {
    ...
    if (entity_vector.empty()) {
      ...
      throw(YInvalidArgumentError{
          "Failed to get 'entity' node from subscribe RPC"});
    }
    DataNode* entity_node = entity_vector[0].get();
    sub.path = new gnmi::Path();
    populate_path_from_payload(sub.path, entity_node->get_value(),
                               *root_schema);
    ...
    if (!interval_dn.empty()) {
      ...
      sub.sample_interval =
          strtoull(interval_node->get_value().c_str(), NULL, 0);
    }
    ...
    if (!suppress_dn.empty()) {
      ...
      if (suppress_node->get_value() == "false") sub.suppress_redundant = false;
    }
    ...
    if (!heartbeat_dn.empty()) {
      ...
      sub.heartbeat_interval =
          strtoull(heartbeat_node->get_value().c_str(), NULL, 0);
    }

    sub_list.push_back(sub);
  }

  client->execute_subscribe_operation(sub_list, qos, mode, encoding, out_func,
                                      poll_func);
...
gNMIClient& gNMISession::get_client() const { return *client; }
