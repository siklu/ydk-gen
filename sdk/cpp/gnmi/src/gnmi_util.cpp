...
static void parse_entity_children(Entity& entity,
                                  vector<PathElem>& path_container);
...
static void parse_entity(Entity& entity, vector<PathElem>& path_container) {
  ...
  if (p != std::string::npos) {
    ...
    for (const pair<string, LeafData>& name_value : path.value_paths) {
      ...
      if (leaf_data.is_set) {
        YLOG_DEBUG("Creating key {} with value: '{}'", name_value.first,
                   leaf_data.value);
...
void parse_entity_to_prefix_and_paths(Entity& entity,
...
static void parse_entity(Entity& entity, gnmi::Path* path) {
  ...
  for (auto key : keys) {
    ...
    YLOG_DEBUG("gnmi_util::parse_entity: Adding key value: '{}:{}'", key.first,
               key.second);
    (*key_map)[key.first] = key.second;
  }
  for (auto leaf : leafs) {
    YLOG_DEBUG("gnmi_util::parse_entity: Adding elem for YLeaf: '{}'",
               leaf.first);
    gnmi::PathElem* elem = path->add_elem();
...
static void parse_entity_children(Entity& entity, gnmi::Path* path) {
  ...
  if (children.size() == 0) {
    ...
    if (entity.has_operation()) {
      std::vector<std::pair<std::string, LeafData> > leaf_data =
          entity.get_name_leaf_data();
      for (auto ld : leaf_data) {
        if (ld.second.yfilter != YFilter::not_set) {
          YLOG_DEBUG(
              "gnmi_util::parse_entity_children: Adding elem for YLeaf: '{}'",
              ld.first);
          gnmi::PathElem* elem = path->add_elem();
  ...
  for (auto const& child : children) {
    if (child.second == nullptr) continue;
    YLOG_DEBUG("");
    YLOG_DEBUG("gnmi_util::parse_entity_children: Looking at child '{}': '{}'",
               child.first,
               get_entity_path(*(child.second), child.second->parent).path);
    if (child.second->has_operation() || child.second->has_data() ||
        child.second->is_presence_container)
...
static void add_path_elem(gnmi::Path* path, string s) {
  ...
  if (p != std::string::npos) {
    ...
    while (open_bracket_pos != string::npos) {
      ...
      string key_name = key_path.substr(open_bracket_pos + 1,
                                        equal_pos - open_bracket_pos - 1);
      string key_value =
          key_path.substr(equal_pos + 2, close_bracket_pos - equal_pos - 3);
  ...
  for (auto key : keys) {
    auto key_map = elem->mutable_key();
    YLOG_DEBUG("gnmi_util::add_path_elem: Adding key value: '{}:{}'", key.first,
               key.second);
    (*key_map)[key.first] = key.second;
  }
}
...
void parse_entity_prefix(Entity& entity, gnmi::Path* path) {
  ...
  for (size_t i = 1; i < segments.size(); i++) {
    if (segments[i] == segment_path) break;
...
void parse_prefix_to_path(const string& prefix, gnmi::Path* path) {
  ...
  if (p != string::npos) {
    ...
    YLOG_DEBUG("parse_prefix_to_path: Got data node path prefix: '{}:{}'", mod,
               con);
    path->set_origin(mod);
    add_path_elem(path, con);
  } else {
    YLOG_DEBUG(
        "parse_prefix_to_path: Got unexpected data node path: '{}', missing "
        "prefix.",
        prefix);
...
static path::DataNode* get_last_datanode(DataNode* dn) {
  ...
  if (!children.empty()) {
    ...
    for (auto child : children) {
      ...
      if (!child_is_key) return get_last_datanode(child.get());
    }
  }
  return dn;
}
