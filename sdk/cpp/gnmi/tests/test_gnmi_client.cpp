...
TEST_CASE("gnmi_secure_server") {
  ...
  string gnmi_server_sert =
      model_dir.substr(0, pos) + "test/gnmi_server/keys/ems.pem";

  gNMIClient client("127.0.0.1", 50051, "admin", "admin", gnmi_server_sert);

  vector<string> caps = client.get_capabilities();
  for (auto cap : caps) cout << cap << endl;
}

/* To Do
TEST_CASE("gnmi_xr")	// Not running
{
    gNMIClient client(grpc::CreateChannel(address,
grpc::InsecureChannelCredentials()), "admin", "admin");
    ...
    string reply =
client.execute_wrapper(R"("rpc":{"ietf-netconf:get-config":{"source":{"running":[null]},"filter":"{\"openconfig-bgp:bgp\":{}}"}})",
"read", true);
...
grpc::InsecureChannelCredentials()));
    ...
    string reply = client.execute_wrapper(
     R"("rpc":{"ietf-netconf:edit-config":{"target":{"running":[null]},"error-option":"rollback-on-error","config":"{\"openconfig-bgp:bgp\":{\"global\":{\"config\":{\"as\":65172}},\"neighbors\":{\"neighbor\":[{\"neighbor-address\":\"172.16.255.2\",\"config\":{\"neighbor-address\":\"172.16.255.2\",\"peer-as\":65172}}]}}}"}})",
"create", true); REQUIRE(NULL != strstr(reply.c_str(), "Success"));
...
"read", true);
    ...
    reply =
client.execute_wrapper(R"("rpc":{"ietf-netconf:edit-config":{"target":{"running":[null]},"error-option":"rollback-on-error","config":"{\"openconfig-bgp:bgp\":{}}"}})",
"delete", true); REQUIRE(NULL != strstr(reply.c_str(), "Success"));
...
{
    gNMIClient client(grpc::CreateChannel("",
grpc::InsecureChannelCredentials()));

    try
    {
        string s = client.execute_wrapper(
R"("rpc":{"ietf-netconf:edit-config":{"target":{"running":[null]},"error-option":"rollback-on-error","config":"{\"openconfig-bgp:bgp\":{\"global\":{\"config\":{\"as\":65172,\"router-id\":\"1.2.3.4\"}},\"neighbors\":{\"neighbor\":[{\"neighbor-address\":\"6.7.8.9\",\"config\":{\"local-as\":65001,\"neighbor-address\":\"6.7.8.9\",\"peer-as\":65001,\"peer-group\":\"IBGP\"}}]},\"peer-groups\":{\"peer-group\":[{\"peer-group-name\":\"IBGP\",\"config\":{\"description\":\"test
description\",\"local-as\":65001,\"peer-as\":65001,\"peer-group-name\":\"IBGP\"}}]}}}"}})",
"create", true); std::cout << "s: " << s << std::endl; REQUIRE(s == "");
    }
    catch(YError & e)
    {
...
grpc::InsecureChannelCredentials()));
    ...
    {
        string reply =
client.execute_wrapper(R"("rpc":{"ietf-netconf:get-config":{"source":{"running":[null]},"filter":"{\"testing}"}})",
"read", true); std::cout << "reply: " << reply << std::endl;
        //REQUIRE(NULL != strstr(reply.c_str(), "parse error"));
    }
    catch (YCPPError & e)
    {
        std::cout << e.err_msg << std::endl;
        //REQUIRE(e.err_msg=="parse error - unexpected '{'; expected string
literal");
    }
    //REQUIRE(result == ok);
}*/
