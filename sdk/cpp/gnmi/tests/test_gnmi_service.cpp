...
static string int_update =
    R"([{\"name\":\"Loopback10\",\"config\":{\"name\":\"Loopback10\",\"description\":\"Test\"}}])";

static string bgp_update =
    R"([{\"neighbor-address\":\"172.16.255.2\",\"config\":{\"neighbor-address\":\"172.16.255.2\",\"peer-as\":65172}}])";
...
TEST_CASE("gnmi_service_subscribe") {
  ...
  gs.subscribe(provider, subscription, 10, "ONCE", "JSON_IETF",
...
TEST_CASE("gnmi_service_subscribe_multiples") {
  ...
  // Subscribe
  gs.subscribe(provider, subscription_list, 10, "ONCE", "JSON_IETF",
               gnmi_service_subscribe_multiples_callback);
}

static int counter;

void set_counter(int max) { counter = max; }
...
TEST_CASE("gnmi_service_poll_subscribe") {
  ...
  set_counter(2);
  gs.subscribe(provider, subscription, 10, "POLL", "JSON_IETF",
               gnmi_service_subscribe_callback, counter_poll_request);
}
...
TEST_CASE("gnmi_service_stream_subscribe") {
  ...
  gs.subscribe(provider, subscription, 10, "STREAM", "JSON_IETF",
               gnmi_service_subscribe_callback);
}
...
TEST_CASE("gnmi_service_create") {
  ...
  REQUIRE(entity2string(get_reply, provider.get_session().get_root_schema()) ==
          expected);
}
...
TEST_CASE("gnmi_service_get_container") {
  ...
  REQUIRE(get_reply != nullptr);
  auto ifc_config =
      dynamic_cast<openconfig_interfaces::Interfaces::Interface::Config*>(
          get_reply.get());
