...
TEST_CASE("test_gnmi_entity_leaf_to_path") {
  auto ifc =
      std::make_shared<ydktest::openconfig_interfaces::Interfaces::Interface>();
  ifc->name = "Loopback10";
...
TEST_CASE("test_gnmi_entity_key_and_leaf_to_path") {
  auto l_1 =
      std::make_shared<ydktest::ydktest_sanity::Runner::TwoList::Ldata>();
  l_1->number = 11;
...
TEST_CASE("test_gnmi_entity_key_and_leaf_to_path2") {
  ...
  auto s_11 = std::make_shared<
      ydktest::ydktest_sanity::Runner::TwoList::Ldata::Subl1>();
  s_11->number = 211;
...
TEST_CASE("test_gnmi_datanode_to_path_2") {
  ...
  auto& two_pr = runner.create_datanode(
      "two-key-list[first='GigabitEthernet0/0/0/0'][second='222']");
...
TEST_CASE("gnmi_test_json_payload") {
  ...
  std::string rpc_json =
      s.encode(edit_config->get_input_node(), ydk::EncodingFormat::JSON, false);
...
TEST_CASE("gnmi_rpc_subscribe") {
  ...
  const ydk::path::gNMISession& session =
      dynamic_cast<const ydk::path::gNMISession&>(provider.get_session());
  ydk::path::RootSchemaNode& schema = session.get_root_schema();
  ...
  auto& subscription =
      rpc->get_input_node().create_datanode("subscription", "");
  ...
  auto& int_subscription =
      subscription.create_datanode("subscription-list[alias='int']", "");
  ...
  auto& bgp_subscription =
      subscription.create_datanode("subscription-list[alias='bgp']", "");
  bgp_subscription.create_datanode("entity", bgp_json);
  // bgp_subscription.create_datanode("subscription-mode", "ON_CHANGE");
  // // can be skipped as default
  bgp_subscription.create_datanode("sample-interval", "20000000");
  // bgp_subscription.create_datanode("suppress-redundant", "true");  	// can
  // be skipped as default
  // bgp_subscription.create_datanode("heartbeat-interval", "2000000000");// can
  // be skipped as it is calculated from sample-interval

  // auto json = codec.encode(rpc->get_input_node(), ydk::EncodingFormat::JSON,
  // true); std::cout << "Built RPC:" << std::endl << json << std::endl;
...
TEST_CASE("gnmi_rpc_set_get_bgp") {
  ...
  auto& int_read =
      schema.create_datanode("openconfig-interfaces:interfaces", "");
  ...
  read_rpc->get_input_node().create_datanode("request[alias='bgp']/entity",
                                             json_bgp);
  read_rpc->get_input_node().create_datanode("request[alias='int']/entity",
                                             json_int);
